...
| `verify124.js` | 第40 | URL短縮辞書v12の2種規則リント(「&キー名=」+「キー値」の形状検査・ビルダー出力148キーとの相互網羅・壊した辞書での自己テスト)+発行済みv11短縮URLの凍結標本復号(`tests/data/qp-v11-sample.json`)+スマホ幅でレイヤーリスト縦4行・短縮URLの5ケース往復(位置情報の日時あり/なし・宙の窓・辻検索・辻メッシュ)・v12短縮URLからの状態復元 |
...
- APP_VERSIONの版数ピンは**最新のverifyのみ**に置きます(第24ラウンド〜。旧verifyのピンを毎回更新する保守を廃止。現在のピンはverify124)
