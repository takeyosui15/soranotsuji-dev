...
| `verify119.js` | 第35 | スマホ辻メッシュ不具合修正(全検索エリアでキャンバス最大辺2048px以下[旧3×3=6144/5×5=5120]・再検索3回とも毎回visible+旧objectURLのrevoke・実チェックボックスOFF→ONの再描画なし復帰・attributionのⓘ[compact]・setDataが天体数に依らずリセット+一括の2回)+監査第2弾v1.37.1(タイルキャッシュLRU上限の追い出し実測・優辻ピンmouseenterのタッチガード+ツールチップoffset毎回設定・検索完了後のワーカープール解放[ソース検査]) |
