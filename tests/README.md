...
| `verify103.js` | 第21 | 宙検索フェーズ2(メニュー拡張[重み8+ラジオ2組+時間帯4+File取得]・プリセット連動と自動カスタム・専用パネル18列・ソート▲▼・行クリック[日時移動+詳細リスト+地図扇形/標本点の雲量着色]・排他[辻検索⇄宙検索]・CSV44列BOM・URL v10往復。ローカル完結=route abortで外部遮断) |
