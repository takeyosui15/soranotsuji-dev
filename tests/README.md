...
| `verify117.js` | 第33 | 予測系機能の一時封鎖+アイコン修正(既定=宙検索/My宙検索メニューと宙断面ボタン非表示・実行ガード・Myセット4シート・Myセット説明文の修正・リリース対象の入り口は表示・⌖/レイヤアイコンの中央±1.5px実測・?forecast=1で3機能表示+5シート+宙断面が開く) |
...
- APP_VERSIONの版数ピンは**最新のverifyのみ**に置きます(第24ラウンド〜。旧verifyのピンを毎回更新する保守を廃止。現在のピンはverify117)
- 第33ラウンドの機能封鎖により、封鎖対象を検証するverify(100/101/103/104/105/106/107/108/112/116)は`?forecast=1`を付けて実行します(封鎖の検証はverify117)
