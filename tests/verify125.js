...
check('M0 辞書は21版(v21が最新。第62=曜日・第80=月間・第88=大気差/気象/基本オプション・第91=天体色/線種・第96=花火頻度・第108=天体色既定値ペア+4キー・第116=写真テクスチャ・第128=辻メッシュ精度○の既定値ペア)', qp.VERSIONS.length === 21, `versions=${qp.VERSIONS.length}`);
...
{
  const g = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'qp-v21-golden.json'), 'utf8'));
  ...
  const g20 = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'qp-v20-golden.json'), 'utf8'));
  check('M4 v20発行の旧ゴールデンの復号が今も元文字列へ戻る(発行済みURLの保証)', qp.dec(g20.golden) === g20.long);
  const e = qp.enc(g.long);
  check('M4 エンコード出力がゴールデンとバイト一致(v21辞書の決定性)', e === g.golden,
