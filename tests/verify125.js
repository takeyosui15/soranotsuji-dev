...
check('M0 辞書は15版(v15が最新。第62=曜日16キー・第80=月間フィルタ26キー追加)', qp.VERSIONS.length === 15, `versions=${qp.VERSIONS.length}`);
...
{
  const g = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'qp-v15-golden.json'), 'utf8'));
  // v13/v14発行URLの凍結保証(復号のみ): 旧ゴールデンが今も元文字列へ戻ること
  ...
  check('M4 エンコード出力がゴールデンとバイト一致(v15辞書の決定性)', e === g.golden,
