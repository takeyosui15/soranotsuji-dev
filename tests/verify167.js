// 第117ラウンド検証: v1.86.0への追補(リリース前) — ①可視判定ポップアップ注記の版数表記を削除
// ②全天儀の天体軌跡線を方位線(中心→天体)と同じ太さへ(トーラス+背面破線3本重ね=_mwTrajCircle)
...
// ---- V0: 版数ピン(最新の検証が持つ。第117はリリース前の追補なので1.86.0のまま) ----
check('V0 版数ピン 1.86.0+Version Historyに第117の追補', /APP_VERSION = '1\.86\.0'/.test(src) && (src.includes('⑥第117ラウンド') || !!process.argv[2]));
...
check('S2 _mwTrajCircle(トーラス0.0025R+破線3本)+方位線も0.0025R+旧ヘルパー廃止',
  src.includes('function _mwTrajCircle') &&
  /TorusGeometry\(R \* Math\.cos\(decR\), 0\.0025 \* _MW_R/.test(src) &&
...
(async()=>{
  ...
  // T1: 全天儀の軌跡線の実測(太さ=方位線と同一・破線3本・原点保持=前面判定の基準が天球中心)
  {
    const r=await p.evaluate(async()=>{
      ...
      out.equal=out.tubeR!==null&&out.azR!==null&&Math.abs(out.tubeR-out.azR)<1e-12;
    ...
    check('T1 軌跡=トーラス(表示天体ぶん)+太さが方位線と同一', r.trajN>0 && r.trajN===r.visN && r.allSameTube && r.azN>0 && r.equal, JSON.stringify(r));
