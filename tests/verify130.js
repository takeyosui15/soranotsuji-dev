...
check('R1 観測点の初期値=東京タワーの建物の外(35.658595, 139.745335)',
    src.includes('DEFAULT_START = { lat: 35.658595, lng: 139.745335, elev: 18.5, height: 150.0 }'));
...
(async () => {
    ...
    // R2: 起動時の観測点=新初期値(start.elevはAPI標高18.5+高さ150=168.5の合成値)
    {
        ...
        check('R2 起動時のappState.start=新初期値(API標高18.5は実測同値・elev=18.5+高さ150)',
            r.lat === 35.658595 && r.lng === 139.745335 && r.api === 18.5 && r.elev === 168.5, JSON.stringify(r));
    }
