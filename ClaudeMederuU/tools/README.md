...
| Koushiプレビュー拡張 | `ClaudeMederuU/tools/vscode-koushi/` | VSCodeの標準MarkdownプレビューでKoushi記法(```koushiフェンス)を表として描くmarkdown-itプラグイン拡張+編集画面のトークンハイライト(TextMate文法注入。第67)。マトリックスCL・状態遷移表を目で確認するため(第66ラウンド・依頼者GO)。レンダラはkoushi.jsの同梱コピー(正はtools/koushi.js・ドリフトはテストが検知) | インストール: `koushi-preview-0.2.0.vsix`をVSCodeへ(手順は同フォルダREADME)。検証: `node ClaudeMederuU/tools/vscode-koushi/extension.test.js`(12チェック)。レンダラ更新時: cpで同期→テスト→`npx @vscode/vsce package --allow-missing-repository`でvsix再生成 |
