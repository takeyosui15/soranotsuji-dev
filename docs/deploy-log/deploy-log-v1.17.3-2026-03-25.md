...
- [x] 今回のデプロイで機能追加されたところをリストアップする。
  - 1. Hom/推山ボタンでリセット/呼び出し時の地図のズームを解除
...
```bash
git add -A
git commit -m "deploy: soranotsuji-dev コミット 13af460 からデプロイ (v1.17.3)"
...
```
デプロイ日時: 2026-03-25 11:05
バージョン: v1.17.3
デプロイ元コミット: 13af460 (soranotsuji-dev)
デプロイ先コミット: f2f6599 (soranotsuji)
確認結果: OK
```
