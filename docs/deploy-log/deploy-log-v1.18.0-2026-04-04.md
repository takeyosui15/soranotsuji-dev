...
- [x] 今回のデプロイで機能追加されたところをリストアップする。
  - 1. マーカー色青赤、既定表示天体複数追加、encodeURL.html追加、URL取得ボタン×2追加
- [x] 上記の機能は、組み込みの**ヘルプ**に内容が反映されているかを確認する。
  - [x] 1. 表示天体、URLボタンについて確認済み
- [x] バージョン番号が更新されている。
...
```bash
git add -A
git commit -m "deploy: soranotsuji-dev コミット 693eae5 からデプロイ (v1.18.0)"
...
```
デプロイ日時: 2026-04-04 16:31
バージョン: v1.18.0
デプロイ元コミット: 693eae5 (soranotsuji-dev)
デプロイ先コミット: 32c2c6f (soranotsuji)
確認結果: OK
```
