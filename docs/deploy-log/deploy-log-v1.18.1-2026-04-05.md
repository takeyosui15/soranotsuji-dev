...
- [x] 今回のデプロイで機能追加されたところをリストアップする。
  - 1. URL取得形式を3種類(日時固定、日時半固定、アクセス日時)に修正、マーカー位置を微調整
...
```bash
git add -A
git commit -m "deploy: soranotsuji-dev コミット d6439ed からデプロイ (v1.18.1)"
...
```
デプロイ日時: 2026-04-05 22:07
バージョン: v1.18.1
デプロイ元コミット: d6439ed (soranotsuji-dev)
デプロイ先コミット: 0537924 (soranotsuji)
確認結果: OK
```
