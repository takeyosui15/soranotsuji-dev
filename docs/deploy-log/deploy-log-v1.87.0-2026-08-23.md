...
- [x] (!!編集箇所あり!!) `script.js` 冒頭のバージョン番号を確認
...
```bash
git add -A
git commit -m "deploy: soranotsuji-dev コミット 984c9c4 からデプロイ (v1.87.0)"
# ↑ abc1234 はStep 1で記録したコミットハッシュに置き換え
# ↑ vX.Y.Z はStep 1で確認したバージョンに置き換え
git push origin main
```

- [x] (!!編集箇所あり!!) 本番リポジトリにもバージョンタグを付与
...
```
デプロイ日時: 2026-08-23 15:25
バージョン: v1.87.0
デプロイ元コミット: 984c9c4 (soranotsuji-dev)
デプロイ先コミット: 55c34f1 (soranotsuji)
確認結果: OK
```
