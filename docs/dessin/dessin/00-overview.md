    ...
    - 状態遷移
      1. ①一定間隔で、ログインチェック
        1. ログイン状態をアイコンで表示
          1. 未ログイン:🈚️
          2. ログイン中:🕛/⭕️/❌/👍/👎のいずれか
            1. ②soranotsuji_app.jsonの破損状態のチェック(注1)
            ...
            3. ④soranotsuji_app.jsonの更新(手動:😢/👎/👍アイコンを押下時)
              1. ローカルストレージを更新開始
                1. ローカルストレージの更新中:🕛
              2. ローカルストレージを更新完了:👍
      - 注1:自動同期を実現する前段階なので、今は、実装しない。
    - 動作
      - ①:一定間隔(1分くらい)で、ログインされているか、されていないかをチェックする。
        - ログイン状態をアイコンで表示する。
          - 未ログイン:🈚️
            - アイコンを押下時の動作
              - Googleドライブへのログインフォームが表示される。
          - それ以外:🕛/⭕️/❌/😢/👎/👍
      - ②:soranotsuji_app.jsonの破損状態のチェック(注1)
      - ③soranotsuji_app.jsonの更新チェック
        - ファイルが存在するかと、ファイルのスキーマバージョン+更新日時を確認する。
          - 一定間隔(1分くらい)で、更新チェックのみをする。
