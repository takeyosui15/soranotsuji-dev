...
発行(buildCommonUrlParamsのprofile)をボタン毎に絞る。**復元側は変わらない**(有るキーだけを適用し、無いキーは開いた側の既定値のまま)。
...
- 既定値の省略: DEFAULT_BODIESの既定(色・線種)と同じ天体は**キー自体を発行しない**。
  - 全て既定のままなら44キーが1つも付かず、URLの長さは今と変わらない。
  - 表示/非表示に関わらず、既定から変えた天体は発行する(非表示天体の色も相手の画面で再現するため)。
  - 省略キーの復元は「復元時点のアプリの既定」になる(将来既定色を変えたら省略URLの見た目も変わる。
    他の既定値キーと同じ扱い=許容)。
- 復元: `bodyColor`/`bodyDash`接頭辞のキーを走査し、一致する組込天体のIDだけに適用(未知IDは無視)。
  復元後は表示天体メニューのUIへ同期。
- 短縮URL辞書(v17): 22天体×2属性=44個の「&キー名=」シードを新版に積む(既定は省略される設計のため
  第3規則の既定値ペアは不要)。色値の`%23`は既存シードが受ける。既存版(v16以前)は凍結のまま。
