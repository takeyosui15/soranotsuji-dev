...
* **「辻ライン」機能 (Diamond/Pearl Line)**:
    ...
    * 5分刻みのマーカーにより、いつ・どこで見られるかが一目で判明。
    * 地球の曲率と標高差を考慮した高精度な計算。
* **「辻検索」機能**:
    * 向こう1年間で観測点が目的点上にくる日付をリストアップします。
    * リストアップされた中で、検索精度を◎○△-で分かりやすく表示します。
    * 目的点に指定したズレ量のオフセットを指定して検索できます。
    * 月齢フィルタで特定の月齢付近の結果を、精度フィルタで精度記号(◎○△-)ごとの結果を絞り込めます。
* **断面標高図**: 観測点から目的点までの地形断面図を表示し、視認性を確認可能。
* **My天体**: 任意の赤経・赤緯を入力して、お好みの星を追跡可能（LocalStorage保存）。
* **My観測点 / My目的点**: 複数の観測点・目的点を登録・管理し、位置反映やCSV入出力が可能。地図上にマーカー表示。
* **My辻検索**: 辻検索の条件を複数登録し、一括計算・CSV結果出力ができる。メモ付き管理に対応。
* **バックアップ / インポート**: 全設定データ（Homeボタン、推し山、表示天体、My天体、My観測点、My目的点、My辻検索、設定）をJSON形式で一括バックアップ・インポート。デバイス間のデータ移行に対応。
...
3.  **「辻検索」をする**: 「ダイヤモンド〇〇」や「パール〇〇」になる日時がリストアップされます。
