...
// 世代管理 (連続呼び出し時の競合を防ぐ)
let dpLineGeneration = 0;

async function updateDPLines() {
    const generation = ++dpLineGeneration;
    // 既存ワーカーをクリーンアップ (前回の計算が残っていれば中断)
    dpLineActiveWorkers.forEach(w => { try { w.terminate(); } catch(_) {} });
    dpLineActiveWorkers = [];
    dpLayer.clearLayers();
    ...
    // 各天体について、prev/curr/next 3日分の計算をワーカーで並列実行
    ...
    // 計算が遅延している間に新しい呼び出しがあった場合は描画しない
    if (generation !== dpLineGeneration) return;
...
// アクティブなDP線ワーカー (キャンセル時に terminate するための参照保持)
let dpLineActiveWorkers = [];
...
async function calculateDPPathPoints(targetDate, body, observer) {
    ...
    // 1時間刻みの粗い可視判定 (メインスレッドで実施: 25回のHorizon呼び出しで済む)
    ...
    const promises = hoursToProcess.map(h => {
        const worker = new Worker('dp-line-worker.js');
        workers.push(worker);
        dpLineActiveWorkers.push(worker);
        return new Promise(resolve => {
            worker.onmessage = (e) => {
                if (e.data && e.data.error) {
                    console.error('dp-line worker error:', e.data.error);
                    resolve({ points: [], hourStart: h });
                } else {
                    resolve(e.data);
                }
            };
            worker.onerror = (err) => {
                console.error('dp-line worker onerror:', err.message || err);
                resolve({ points: [], hourStart: h });
            };
            worker.postMessage({
                body: bodyMsg,
    ...
    dpLineActiveWorkers = dpLineActiveWorkers.filter(w => !workers.includes(w));

    // hourStart 順にソートしてマージ
    results.sort((a, b) => a.hourStart - b.hourStart);
    const path = [];
