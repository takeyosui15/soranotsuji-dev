...
const APP_DEFAULTS = {
    ...
    // ---- 辻検索: 曜日フィルタ(第61ラウンド・デッサン03) ----
    tsujiDowFilter: { def: false },
    tsujiDowMon: { def: false }, tsujiDowTue: { def: false }, tsujiDowWed: { def: false }, tsujiDowThu: { def: false },
    tsujiDowFri: { def: false }, tsujiDowSat: { def: false }, tsujiDowSun: { def: false },
    ...
    // ---- 辻メッシュ検索: 曜日フィルタ(第61ラウンド・デッサン04) ----
    tsujiMeshDowFilter: { def: false },
    tsujiMeshDowMon: { def: false }, tsujiMeshDowTue: { def: false }, tsujiMeshDowWed: { def: false }, tsujiMeshDowThu: { def: false },
    tsujiMeshDowFri: { def: false }, tsujiMeshDowSat: { def: false }, tsujiMeshDowSun: { def: false },
...
let appState = {
    ...
    mySetHome: {          // 既定のセット(ID:0)の行情報 (名称固定・常に端末保持。シート紐付けは可能)
        id: 0, name: '既定のセット', saveMode: 'save', checked: false,
        updatedAt: null, memo: '', sheetId: null, lastSyncSheetTime: null
    },
...
window.onload = async function() {
    ...
    document.getElementById('chk-tsuji-dow-filter').checked = appState.tsujiDowFilter;
    _DOW_DEFS.forEach(([suf, key]) => { document.getElementById(`chk-tsuji-dow-${suf}`).checked = appState['tsujiDow' + key]; });
    updateTsujiDowFilterUI();
    ...
    document.getElementById('chk-tsujimesh-dow-filter').checked = appState.tsujiMeshDowFilter;
    _DOW_DEFS.forEach(([suf, key]) => { document.getElementById(`chk-tsujimesh-dow-${suf}`).checked = appState['tsujiMeshDow' + key]; });
    updateTsujiMeshDowFilterUI();
...
function setupUI() {
    ...
    // 曜日フィルタ(第61ラウンド・デッサン03)
    document.getElementById('chk-tsuji-dow-filter').addEventListener('change', (e) => {
        appState.tsujiDowFilter = e.target.checked;
        updateTsujiDowFilterUI();
        saveAppState();
    });
    _DOW_DEFS.forEach(([suf, key]) => {
        document.getElementById(`chk-tsuji-dow-${suf}`).addEventListener('change', (e) => {
            appState['tsujiDow' + key] = e.target.checked;
            saveAppState();
        });
    });
    ...
    // 辻メッシュ: 曜日フィルタ(第61ラウンド・デッサン04)
    document.getElementById('chk-tsujimesh-dow-filter').addEventListener('change', (e) => {
        appState.tsujiMeshDowFilter = e.target.checked;
        updateTsujiMeshDowFilterUI();
        saveAppState();
    });
    _DOW_DEFS.forEach(([suf, key]) => {
        document.getElementById(`chk-tsujimesh-dow-${suf}`).addEventListener('change', (e) => {
            appState['tsujiMeshDow' + key] = e.target.checked;
            saveAppState();
        });
    });
    ...
    // 宙検索: メニュー+結果パネルのコントロール一式(フェーズ2/3)
...
function buildStateToSave() {
    ...
    return {
        ...
        tsujiDowFilter: appState.tsujiDowFilter,
        tsujiDowMon: appState.tsujiDowMon, tsujiDowTue: appState.tsujiDowTue, tsujiDowWed: appState.tsujiDowWed, tsujiDowThu: appState.tsujiDowThu,
        tsujiDowFri: appState.tsujiDowFri, tsujiDowSat: appState.tsujiDowSat, tsujiDowSun: appState.tsujiDowSun,
        ...
        tsujiMeshDowFilter: appState.tsujiMeshDowFilter,
        tsujiMeshDowMon: appState.tsujiMeshDowMon, tsujiMeshDowTue: appState.tsujiMeshDowTue, tsujiMeshDowWed: appState.tsujiMeshDowWed, tsujiMeshDowThu: appState.tsujiMeshDowThu,
        tsujiMeshDowFri: appState.tsujiMeshDowFri, tsujiMeshDowSat: appState.tsujiMeshDowSat, tsujiMeshDowSun: appState.tsujiMeshDowSun,
        // 宙の窓パラメータ
...
function loadAppState() {
    ...
    if (json) {
        try {
            ...
            if(saved.mySetHome) appState.mySetHome = { ...appState.mySetHome, ...saved.mySetHome, id: 0, name: '既定のセット' };
            ...
            ['tsujiMeshDays', 'tsujiMeshBaseAz', 'tsujiMeshOffsetAz', 'tsujiMeshToleranceAz',
             ...
             'tsujiMeshDowFilter','tsujiMeshDowMon','tsujiMeshDowTue','tsujiMeshDowWed','tsujiMeshDowThu','tsujiMeshDowFri','tsujiMeshDowSat','tsujiMeshDowSun'].forEach(k => {
            ...
            ['tsujiDowFilter','tsujiDowMon','tsujiDowTue','tsujiDowWed','tsujiDowThu','tsujiDowFri','tsujiDowSat','tsujiDowSun'].forEach(k => { if (saved[k] !== undefined) appState[k] = saved[k]; });
...
async function handleLocationInput(val, isStart, fromName) {
    ...
    // - 英字のみ・海外言語のみの入力は変換しない(全角化すると海外地名がOSMで検索できなかった)
    // - 数値のみ(カンマ無し)の入力はsearchLocation側で従来どおり検索スキップ
    //   (OSMが郵便番号と解釈して海外へ飛ぶ実測あり。Zipコード誤認防止は維持)
    const trimmed = val.trim();
    const hasJapanese = /[぀-ヿ㐀-鿿豈-﫿ｦ-ﾟ]/.test(trimmed);
    const fullVal = hasJapanese ? toFullWidth(trimmed) : trimmed;
...
function getMyTsujiFromTsujiSearch() {
    ...
    appState.myTsujiSearches.push({
        ...
        dowFilter: appState.tsujiDowFilter, dowMon: appState.tsujiDowMon, dowTue: appState.tsujiDowTue, dowWed: appState.tsujiDowWed, dowThu: appState.tsujiDowThu, dowFri: appState.tsujiDowFri, dowSat: appState.tsujiDowSat, dowSun: appState.tsujiDowSun,
...
function copyMyTsujiSearchUrl(includeDateTime) {
    ...
    params.set('tsujiDowFilter', t.dowFilter ? 'true' : 'false');
    params.set('tsujiDowMon', t.dowMon ? 'true' : 'false');
    params.set('tsujiDowTue', t.dowTue ? 'true' : 'false');
    params.set('tsujiDowWed', t.dowWed ? 'true' : 'false');
    params.set('tsujiDowThu', t.dowThu ? 'true' : 'false');
...
async function decorateMyTsujiResults(results) {
    ...
    // 標高フィルタ: (観測点ID,目的点ID)毎に1回だけ可視判定 (entry.elevationOption が ON のもの)
    ...
    for (const r of results) {
        ...
        // 標高フィルタ: 可視判定の結果(OK/NG/-)とOK/NGフィルタ
        ...
        // 曜日フィルタ(第61ラウンド・デッサン10): 行の設定(r.tsuji.dow*)で絞り込む
        if (!_dowFilterAllows(r.tsuji, dt.getDay())) continue;
        ...
        decorated.push({ ...r, symbol, dateStr, dowStr, timeStr, moonAge, moonIcon, moonIllum, angularRadius, timeCategory,
                 elevationStatus, sunriseStr, sunsetStr, moonriseStr, moonsetStr, tw });
    }
    return decorated;
}
...
async function runBatchMyTsujiSearch() {
    ...
    _tsujiResRaw = { kind: 'mytsuji', decorated };
    _resCtlSet('tsujires', _resCtlAllOff(), decorated.some(r => r.elevationStatus === 'OK' || r.elevationStatus === 'NG'));
    _myTsujiRenderResults();
}

/** My辻検索結果の表示(第62ラウンド: 結果コントロールの追加フィルタで再導出できるよう切り出し)。
 *  各行の検索は行ごとの設定で実行・フィルタ済みのため、コントロールは「追加の絞り込み」(初期値=全オフ)。 */
function _myTsujiRenderResults() {
    const R = _tsujiResRaw;
    if (!R || R.kind !== 'mytsuji') return;
    const F = _resCtlRead('tsujires');
    const content = document.getElementById('tsujisearch-content');
    const statusEl = document.getElementById('tsujisearch-status');
    content.innerHTML = '';
    const rows = R.decorated.filter(r => _myTsujiResPass(r, F));
    statusEl.textContent = `${rows.length}件`;
    if (rows.length === 0) {
        content.innerHTML = '<div style="padding:8px;color:#999;">フィルタの結果、該当する日時はありません</div>';
        return;
    }
    ...
    rows.forEach(r => tbody.appendChild(renderMyTsujiResultRow(r)));
...
function renderMyTsujiSearches() {
    ...
    list.forEach((t, idx) => {
        ...
        row.innerHTML = `
            ...
            <hr class="tsujisearch-separator">
            <div class="control-row left-row">
                <input type="checkbox" class="body-checkbox mytsuji-dow-filter" data-id="${t.id}" ${t.dowFilter ? 'checked' : ''}>
                <label>曜日フィルタ</label>
            </div>
            <div class="control-row left-row">
                <input type="checkbox" class="body-checkbox mytsuji-dow-mon" data-id="${t.id}" ${t.dowMon ? 'checked' : ''} ${t.dowFilter ? '' : 'disabled'}> <label class="tsuji-acc-label">:月</label>
                <input type="checkbox" class="body-checkbox mytsuji-dow-tue" data-id="${t.id}" ${t.dowTue ? 'checked' : ''} ${t.dowFilter ? '' : 'disabled'}> <label class="tsuji-acc-label">:火</label>
                <input type="checkbox" class="body-checkbox mytsuji-dow-wed" data-id="${t.id}" ${t.dowWed ? 'checked' : ''} ${t.dowFilter ? '' : 'disabled'}> <label class="tsuji-acc-label">:水</label>
                <input type="checkbox" class="body-checkbox mytsuji-dow-thu" data-id="${t.id}" ${t.dowThu ? 'checked' : ''} ${t.dowFilter ? '' : 'disabled'}> <label class="tsuji-acc-label">:木</label>
            </div>
            <div class="control-row left-row">
                <input type="checkbox" class="body-checkbox mytsuji-dow-fri" data-id="${t.id}" ${t.dowFri ? 'checked' : ''} ${t.dowFilter ? '' : 'disabled'}> <label class="tsuji-acc-label">:金</label>
                <input type="checkbox" class="body-checkbox mytsuji-dow-sat" data-id="${t.id}" ${t.dowSat ? 'checked' : ''} ${t.dowFilter ? '' : 'disabled'}> <label class="tsuji-acc-label">:土</label>
                <input type="checkbox" class="body-checkbox mytsuji-dow-sun" data-id="${t.id}" ${t.dowSun ? 'checked' : ''} ${t.dowFilter ? '' : 'disabled'}> <label class="tsuji-acc-label">:日</label>
            </div>
            <hr class="tsujisearch-separator">
            <div class="control-row left-row">
        ...
        // 曜日フィルタ(第61ラウンド・デッサン10)
        onChange('mytsuji-dow-filter', e => {
            t.dowFilter = e.target.checked;
            _DOW_DEFS.forEach(([suf]) => { row.querySelector('.mytsuji-dow-' + suf).disabled = !t.dowFilter; });
            saveAppState(); setMyTsujiDirty(true);
        });
        _DOW_DEFS.forEach(([suf, key]) => {
            onChange('mytsuji-dow-' + suf, e => { t['dow' + key] = e.target.checked; saveAppState(); setMyTsujiDirty(true); });
        });
...
// ==== 結果コントロールメニュー(第62ラウンド・デッサン03/04/10) ====
// 辻検索/My辻検索(共有パネル=tsujires)と辻メッシュ(tsujimeshres)の検索結果へ、フィルタ再適用+File出力を
// 提供する。値は「検索実行時のスナップショットの独立コピー」としてコントロールのDOMに展開し、変更のたびに
// DOMから読み出して保持した生結果から再導出する(再検索なし・検索メニューへは書き戻さない)。
// My辻検索は各行が行ごとの設定で検索済みのため、コントロールは追加の絞り込み(初期値=全オフ)。
let _tsujiResRaw = null;   // 辻検索: {kind:'tsuji', totalResults, elevStatus, ctx} / My辻: {kind:'mytsuji', decorated}
let _tmResRaw = null;      // 辻メッシュ: {allBodyEvents, visFlags, elevOn}

/** コントロールのDOMからフィルタ値Fを読む(Fのフィールド名は時間フィルタ関数と互換) */
function _resCtlRead(P) {
    ...
    const str = (id, def) => { const el = document.getElementById(id); return el && el.value ? el.value : def; };
    const radio = (name, def) => { const el = document.querySelector(`input[name="${name}"]:checked`); return el ? el.value : def; };
    const F = {
        moonFilter: chk(`chk-${P}-moon-filter`), moonBase: num(`input-${P}-moon-base`, 14.8), moonTolerance: num(`input-${P}-moon-tolerance`, 2),
        timeFilter: chk(`chk-${P}-time-filter`),
        startMode: radio(`${P}-start-mode`, 'sunset'), startTime: str(`input-${P}-start-time`, '00:00'),
        startPrePost: chk(`chk-${P}-start-prepost`), startPrePostDir: radio(`${P}-start-prepost-dir`, 'before'), startOffset: str(`input-${P}-start-offset`, '00:00'),
        endMode: radio(`${P}-end-mode`, 'sunrise'), endTime: str(`input-${P}-end-time`, '00:00'),
        endPrePost: chk(`chk-${P}-end-prepost`), endPrePostDir: radio(`${P}-end-prepost-dir`, 'before'), endOffset: str(`input-${P}-end-offset`, '00:00'),
        dowFilter: chk(`chk-${P}-dow-filter`),
        accuracyFilter: chk(`chk-${P}-acc-filter`),
        accDblCircle: chk(`chk-${P}-acc-dbl-circle`), accCircle: chk(`chk-${P}-acc-circle`),
        accTriangle: chk(`chk-${P}-acc-triangle`), accDash: chk(`chk-${P}-acc-dash`),
        elevFilter: chk(`chk-${P}-elev-filter`), elevOK: chk(`chk-${P}-elev-ok`), elevNG: chk(`chk-${P}-elev-ng`),
    };
    _DOW_DEFS.forEach(([suf, key]) => { F['dow' + key] = chk(`chk-${P}-dow-${suf}`); });
    return F;
}

/** フィルタ値FをコントロールのDOMへ展開する(検索完了時のスナップショット)。elevAvail=標高フィルタの有効可否 */
function _resCtlSet(P, F, elevAvail) {
    ...
    setChk(`chk-${P}-moon-filter`, F.moonFilter); setVal(`input-${P}-moon-base`, F.moonBase); setVal(`input-${P}-moon-tolerance`, F.moonTolerance);
    setChk(`chk-${P}-time-filter`, F.timeFilter);
    setRadio(`${P}-start-mode`, F.startMode); setVal(`input-${P}-start-time`, F.startTime);
    setChk(`chk-${P}-start-prepost`, F.startPrePost); setRadio(`${P}-start-prepost-dir`, F.startPrePostDir); setVal(`input-${P}-start-offset`, F.startOffset);
    setRadio(`${P}-end-mode`, F.endMode); setVal(`input-${P}-end-time`, F.endTime);
    ...
    _resCtlUpdateEnable(P);
    if (P === 'tsujires') { const c = document.getElementById('tsujires-ctrl'); if (c) c.classList.remove('hidden'); }
}

/** 検索メニューのappState値からフィルタ値Fを作る(独立コピー)。sp='tsuji'|'tsujiMesh' */
function _resCtlFromAppState(sp) {
    const F = {
        moonFilter: appState[sp + 'MoonFilterEnabled'], moonBase: appState[sp + 'MoonBase'], moonTolerance: appState[sp + 'MoonTolerance'],
        timeFilter: appState[sp + 'TimeFilter'],
        startMode: appState[sp + 'StartMode'], startTime: appState[sp + 'StartTime'],
        startPrePost: appState[sp + 'StartPrePost'], startPrePostDir: appState[sp + 'StartPrePostDir'], startOffset: appState[sp + 'StartOffset'],
        endMode: appState[sp + 'EndMode'], endTime: appState[sp + 'EndTime'],
        endPrePost: appState[sp + 'EndPrePost'], endPrePostDir: appState[sp + 'EndPrePostDir'], endOffset: appState[sp + 'EndOffset'],
        dowFilter: appState[sp + 'DowFilter'],
    };
    _DOW_DEFS.forEach(([, key]) => { F['dow' + key] = appState[sp + 'Dow' + key]; });
    if (sp === 'tsuji') {
        F.accuracyFilter = appState.tsujiAccuracyFilterEnabled;
        F.accDblCircle = appState.tsujiAccDblCircle; F.accCircle = appState.tsujiAccCircle;
...
/** 全オフのフィルタ値(My辻検索の追加絞り込みの初期値) */
function _resCtlAllOff() {
    const F = {
        moonFilter: false, moonBase: 14.8, moonTolerance: 2,
        ...
        dowFilter: false, accuracyFilter: false, accDblCircle: false, accCircle: false, accTriangle: false, accDash: false,
        elevFilter: false, elevOK: false, elevNG: false,
    };
    _DOW_DEFS.forEach(([, key]) => { F['dow' + key] = false; });
    return F;
}

/** コントロールの活性/非活性を現在のチェック状態から更新する */
function _resCtlUpdateEnable(P) {
    const chk = (id) => { const el = document.getElementById(id); return !!(el && el.checked); };
    const dis = (id, v) => { const el = document.getElementById(id); if (el) el.disabled = v; };
    const moonOn = chk(`chk-${P}-moon-filter`);
    dis(`input-${P}-moon-base`, !moonOn); dis(`input-${P}-moon-tolerance`, !moonOn);
    const timeOn = chk(`chk-${P}-time-filter`);
    document.querySelectorAll(`#${P}-time-filter-groups .${P}-time-control`).forEach(el => { el.disabled = !timeOn; });
    ['start', 'end'].forEach(g => {
        const pp = timeOn && chk(`chk-${P}-${g}-prepost`);
        document.querySelectorAll(`.${P}-${g}-prepost-control`).forEach(el => { el.disabled = !pp; });
    });
    ...
    const elevOn = elevAvail && chk(`chk-${P}-elev-filter`);
    dis(`chk-${P}-elev-ok`, !elevOn); dis(`chk-${P}-elev-ng`, !elevOn);
}

/** フィルタ変更の再導出ディスパッチ */
function _resCtlReapply(P) {
    if (P === 'tsujires') {
        if (!_tsujiResRaw) return;
        if (_tsujiResRaw.kind === 'tsuji') _tsujiRenderResults();
        else _myTsujiRenderResults();
    } else {
        if (!_tmResRaw || !_tsujiMeshCalc) return;
        const built = _tmBuildRows(_tmResRaw.allBodyEvents, _resCtlRead('tsujimeshres'), _tmResRaw.visFlags, _tmResRaw.elevOn);
        _tsujiMeshSelIdx = -1;
        _tsujiMeshRows = built.rows;
        const st = document.getElementById('tsujimesh-status');
        if (st) st.textContent = `(${built.rows.length}件 / ヒット画素のべ${built.totalPix.toLocaleString()})`;
...
/** My辻検索結果の追加絞り込み判定(decorate済みの行に対する結果コントロールのフィルタ) */
function _myTsujiResPass(r, F) {
    if (F.moonFilter && !isMoonAgeInRange(r.moonAge, F.moonBase, F.moonTolerance)) return false;
    if (F.timeFilter && r.tw && !passesTimeFilter(r.time, r.tw, F)) return false;
    if (!_dowFilterAllows(F, r.time.getDay())) return false;
    if (F.accuracyFilter) {
        const a = [];
        if (F.accDblCircle) a.push('◎');
        if (F.accCircle) a.push('○');
        if (F.accTriangle) a.push('△');
        if (F.accDash) a.push('-');
        if (a.length > 0 && !a.includes(r.symbol)) return false;
    }
    if (F.elevFilter && (F.elevOK || F.elevNG)) {
        const a = [];
        if (F.elevOK) a.push('OK');
        if (F.elevNG) a.push('NG');
        if (!a.includes(r.elevationStatus)) return false;
    }
    return true;
}

/** 結果コントロールのFile出力(tsujires: 辻検索=生結果から再decorate / My辻=絞り込み後の行をそのまま) */
async function _tsujiResFileCsv() {
    const R = _tsujiResRaw;
    if (!R) return;
    const F = _resCtlRead('tsujires');
    if (R.kind === 'mytsuji') {
        const rows = R.decorated.filter(r => _myTsujiResPass(r, F));
        if (!rows.length) return alert('該当する日時はありません');
        await downloadTsujiResultCsv(rows, `soranotsuji-My辻検索結果-${formatFileDateTime()}.csv`);
        return;
    }
    const t = {
        id: '', name: '', memo: '', days: R.ctx.days,
        baseAz: R.ctx.baseAz, baseAlt: R.ctx.baseAlt, offsetAz: R.ctx.offsetAz, offsetAlt: R.ctx.offsetAlt,
        centerMode: R.ctx.searchCenterMode === 'line' ? 'line' : 'point', mwOffsetAngle: R.ctx.mwOffAngle,
        moonFilter: F.moonFilter, moonBase: F.moonBase, moonTolerance: F.moonTolerance,
        accuracyFilter: F.accuracyFilter, accDblCircle: F.accDblCircle, accCircle: F.accCircle, accTriangle: F.accTriangle, accDash: F.accDash,
        elevationOption: F.elevFilter, elevOK: F.elevOK, elevNG: F.elevNG,
        timeFilter: F.timeFilter,
        startMode: F.startMode, startTime: F.startTime, startPrePost: F.startPrePost, startPrePostDir: F.startPrePostDir, startOffset: F.startOffset,
        endMode: F.endMode, endTime: F.endTime, endPrePost: F.endPrePost, endPrePostDir: F.endPrePostDir, endOffset: F.endOffset,
        dowFilter: F.dowFilter, dowMon: F.dowMon, dowTue: F.dowTue, dowWed: F.dowWed, dowThu: F.dowThu, dowFri: F.dowFri, dowSat: F.dowSat, dowSun: F.dowSun,
    };
    const allResults = [];
    _pushMyTsujiResults(allResults, t, { obs: R.ctx.obs, tgt: R.ctx.tgt, bodyResults: R.totalResults });
    const statusEl = document.getElementById('tsujisearch-status');
    if (statusEl) statusEl.innerHTML = '<span class="clock-anim">🕛</span> File出力処理中...';
    const decorated = await decorateMyTsujiResults(allResults);
    if (!decorated.length) { if (statusEl) statusEl.textContent = '0件'; return alert('該当する日時はありません'); }
    await downloadTsujiResultCsv(decorated, `soranotsuji-辻検索結果-${formatFileDateTime()}.csv`);
    if (statusEl) statusEl.textContent = `${decorated.length}件 (CSV出力完了)`;
}

/** 結果コントロールの初期化(HTML生成+配線)。アプリ起動時に1回呼ぶ */
function _resCtlInit() {
    ['tsujires', 'tsujimeshres'].forEach(P => {
        const tg = document.getElementById(`${P}-time-filter-groups`);
        if (tg) tg.innerHTML = buildTimeGroupHtmlFor(P, 'tsuji', 'start') + buildTimeGroupHtmlFor(P, 'tsuji', 'end');
        const ids = [`chk-${P}-moon-filter`, `input-${P}-moon-base`, `input-${P}-moon-tolerance`,
            `chk-${P}-time-filter`, `input-${P}-start-time`, `input-${P}-start-offset`, `chk-${P}-start-prepost`,
            `input-${P}-end-time`, `input-${P}-end-offset`, `chk-${P}-end-prepost`,
            `chk-${P}-dow-filter`, `chk-${P}-acc-filter`,
            `chk-${P}-acc-dbl-circle`, `chk-${P}-acc-circle`, `chk-${P}-acc-triangle`, `chk-${P}-acc-dash`,
            `chk-${P}-elev-filter`, `chk-${P}-elev-ok`, `chk-${P}-elev-ng`];
        _DOW_DEFS.forEach(([suf]) => ids.push(`chk-${P}-dow-${suf}`));
        const onCh = () => { _resCtlUpdateEnable(P); _resCtlReapply(P); };
        ids.forEach(id => { const el = document.getElementById(id); if (el) el.addEventListener('change', onCh); });
        document.querySelectorAll(`input[name="${P}-start-mode"], input[name="${P}-end-mode"], input[name="${P}-start-prepost-dir"], input[name="${P}-end-prepost-dir"]`)
            .forEach(el => el.addEventListener('change', onCh));
    });
    const hdr = document.getElementById('tsujires-ctrl-header');
    if (hdr) hdr.addEventListener('click', () => {
        const ctl = document.getElementById('tsujires-ctrl');
        const body = document.getElementById('tsujires-ctrl-body');
        const arrow = document.getElementById('tsujires-ctrl-arrow');
        const open = body.classList.toggle('hidden');
        ctl.classList.toggle('open', !open);
        if (arrow) arrow.textContent = open ? '▼' : '▲';
    });
    const fbtn = document.getElementById('btn-tsujires-file');
    if (fbtn) fbtn.addEventListener('click', () => { _tsujiResFileCsv().catch(e => { console.error(e); alert('File出力に失敗しました: ' + (e && e.message ? e.message : e)); }); });
    const mbtn = document.getElementById('btn-tsujimeshres-file');
    if (mbtn) mbtn.addEventListener('click', () => {
        if (!_tsujiMeshRows.length) return alert('該当する日時はありません');
        _tmExportMeshRowsCsv(_tsujiMeshRows).catch(e => { console.error(e); alert('CSVの生成に失敗しました: ' + (e && e.message ? e.message : e)); });
    });
}

// ==== 曜日フィルタ(第61ラウンド・デッサン03/04/10) ====
// [チェックボックスidの接尾辞, appState/行フィールドのキー接尾辞, getDay()値] の対応表
const _DOW_DEFS = [['mon', 'Mon', 1], ['tue', 'Tue', 2], ['wed', 'Wed', 3], ['thu', 'Thu', 4],
                   ['fri', 'Fri', 5], ['sat', 'Sat', 6], ['sun', 'Sun', 0]];
/** 曜日フィルタの判定: F={dowFilter, dowMon..dowSun}。無効/全て未チェックは絞らない(精度フィルタと同じ流儀) */
function _dowFilterAllows(F, day) {
    if (!F || !F.dowFilter) return true;
    const allowed = _DOW_DEFS.filter(([, key]) => F['dow' + key]).map(([, , d]) => d);
    if (allowed.length === 0) return true;
    return allowed.includes(day);
}
/** 曜日フィルタのUI状態(活性/非活性)を更新(辻検索メニュー) */
function updateTsujiDowFilterUI() {
    const on = appState.tsujiDowFilter;
    _DOW_DEFS.forEach(([suf]) => { document.getElementById(`chk-tsuji-dow-${suf}`).disabled = !on; });
}
/** 曜日フィルタのUI状態(活性/非活性)を更新(辻メッシュ検索メニュー) */
function updateTsujiMeshDowFilterUI() {
    const on = appState.tsujiMeshDowFilter;
    _DOW_DEFS.forEach(([suf]) => { document.getElementById(`chk-tsujimesh-dow-${suf}`).disabled = !on; });
}
...
/** 辻メッシュ: 標高フィルタのUI状態(OK/NGチェックの入力可否)を更新 */
...
function _tmBuildGradientOverlay() {
    ...
    for (let p2 = 0; p2 < n; p2++) {
        const c = E.start[p2 + 1] - E.start[p2];
        ...
        for (let gx = 0; gx < W; gx++) {
            ...
            for (let sy = 0; sy < S; sy++) {
                for (let sx = 0; sx < S; sx++) {
                    let R2 = cr, G2 = cg, B2 = cb;
                    if (isMax) {
                        // 金色と赤色の斜め縞(グローバル座標で連続)
                        const stripe = ((x0 + sx + y0 + sy) % STRIPE) < (STRIPE >> 1);
                        R2 = stripe ? GOLD[0] : RED[0];
                        G2 = stripe ? GOLD[1] : RED[1];
                        B2 = stripe ? GOLD[2] : RED[2];
                    }
                    const o = ((y0 + sy) * CW + x0 + sx) * 4;
                    data[o] = R2; data[o + 1] = G2; data[o + 2] = B2; data[o + 3] = 230;
                }
            }
        }
    }
...
/** 辻メッシュ標高フィルタ: 対象画素それぞれを観測点として、統一可視判定コア(_visJudgeCore)で
 *  可視判定する(標高グラフの可視判定・辻検索/My辻検索の標高フィルタと完全に同じベース)。
...
async function startTsujiMeshSearch() {
    ...
    tmVisPool.terminateAll();   // 実行中の可視判定(標高フィルタ)もキャンセル
    ...
    // 1b) 標高フィルタ: 許容範囲で絞った対象画素それぞれに、標高グラフと同一のアルゴリズムで
    ...
    // 5) (天体,日)イベント → 結果行へ整形 + フィルタ + 装飾(第62ラウンド: 結果コントロールと共通の再導出)
    _tmResRaw = { allBodyEvents, visFlags: window._tmLastVisFlags, elevOn: elevOptOn };
    _resCtlSet('tsujimeshres', _resCtlFromAppState('tsujiMesh'), elevOptOn);
    const built = _tmBuildRows(allBodyEvents, _resCtlRead('tsujimeshres'), window._tmLastVisFlags, elevOptOn);
    const rows = built.rows;
    const totalPix = built.totalPix;
...
/** 辻メッシュ: (天体,日)イベント→結果行の構築(第62ラウンド: 結果コントロールから再導出できるよう切り出し)。
 *  F=結果コントロールのフィルタ値。visFlags=標高フィルタの可視判定(検索時に計算済み)。elevOn=検索時に標高フィルタON。 */
function _tmBuildRows(allBodyEvents, F, visFlags, elevOn) {
    const C = _tsujiMeshCalc;
    const observer = new Astronomy.Observer(C.observerData.lat, C.observerData.lng, C.observerData.elev);
    const timeFs = F;   // Fは時間フィルタと同名フィールド(startMode..endOffset)を持つ
    ...
    allBodyEvents.forEach(({ body, events }) => {
        events.forEach(ev => {
            ...
            // 月齢フィルタ
            if (F.moonFilter &&
                !isMoonAgeInRange(moonAge, F.moonBase, F.moonTolerance)) return;
            // 時間フィルタ
            if (F.timeFilter && !passesTimeFilter(dt, rs.tw, timeFs)) return;
            // 曜日フィルタ(第61ラウンド)
            if (!_dowFilterAllows(F, dt.getDay())) return;
            const symbol = _tmAccSymbol(rowDist);   // ◎〜◎×128(◎より粗い場合は○△-)
            // 精度フィルタ(◎〜◎×128は常時オン・○△-はチェックされた記号のみ表示)
            if (F.accuracyFilter) {
                const allowedSym = [];
                if (F.accDblCircle) allowedSym.push('◎');
            ...
            const rowElevStatus = (elevOn && visFlags)
                ? (visFlags[ev.bestPix] ? 'OK' : 'NG')
                : '-';
            // 標高フィルタ(結果コントロール): 行の最良画素の可視判定で絞り込む
            if (F.elevFilter && (F.elevOK || F.elevNG)) {
                const allowedElev = [];
                if (F.elevOK) allowedElev.push('OK');
                if (F.elevNG) allowedElev.push('NG');
                if (!allowedElev.includes(rowElevStatus)) return;
            }
            totalPix += ev.total;
            rows.push({
                body, dateObj: dt,
                ...
                elevationStatus: rowElevStatus,   // 行の表示値を出した地点=最良画素の可視判定(標高グラフと同一アルゴリズム)
...
function setupTsujiMeshPanelControls() {
    ...
    _resCtlInit();   // 結果コントロールメニュー(第62ラウンド)
...
async function startTsujiSearch() {
    ...
    // 標高フィルタ: start→end の可視判定を1回計算 (start/end は検索中固定のため全行共通)
    let elevStatus = '-';
    ...
    // 結果コントロールメニュー(第62ラウンド・デッサン03): 生結果+検索時の文脈を保持し、
    // 検索メニューのフィルタ値をコントロールへスナップショット(独立コピー)してから共通の再導出で描画する
    _tsujiResRaw = {
        kind: 'tsuji', totalResults, elevStatus,
        ctx: {
    ...
    _resCtlSet('tsujires', _resCtlFromAppState('tsuji'), elevStatus !== '-');
    _tsujiRenderResults();
}

/** 辻検索の結果表示(第62ラウンド: 結果コントロールから再導出できるよう切り出し)。
 *  _tsujiResRaw(生結果+検索時の文脈)と結果コントロールのフィルタ値(F)から表を描く。 */
function _tsujiRenderResults() {
    const R = _tsujiResRaw;
    if (!R || R.kind !== 'tsuji') return;
    const F = _resCtlRead('tsujires');
    const { totalResults, elevStatus, ctx } = R;
    const statusEl = document.getElementById('tsujisearch-status');
    const contentEl = document.getElementById('tsujisearch-content');
    contentEl.innerHTML = '';
    const observer = new Astronomy.Observer(ctx.observerData.lat, ctx.observerData.lng, ctx.observerData.elev);
    const ctrRef = (r) => tsujiCenterRefPoint(r.azimuth, r.altitude, ctx.baseAz, ctx.baseAlt, ctx.offsetAz, ctx.offsetAlt, ctx.searchCenterMode);
    ...
    totalResults.forEach(({ body, results, limitReached }) => {
        results.forEach(r => {
            ...
            if (F.moonFilter &&
                !isMoonAgeInRange(moonAge, F.moonBase, F.moonTolerance)) {
                return;
            }
            ...
            if (F.accuracyFilter) {
                const allowed = [];
                if (F.accDblCircle) allowed.push('◎');
                if (F.accCircle) allowed.push('○');
                if (F.accTriangle) allowed.push('△');
                if (F.accDash) allowed.push('-');
            ...
            // 曜日フィルタ(第61ラウンド): チェックされた曜日のみ通す
            if (!_dowFilterAllows(F, dt.getDay())) return;

            rowData.push({
                body, symbol, dateStr, dowStr, timeStr, dateObj: dt,
                dist: r.dist, azimuth: r.azimuth, altitude: r.altitude,
                azDiff: azDiffDeg(r.azimuth, ctrRef(r).az),   // 検索中心方位角差(検索中心より右=正)
                altDiff: r.altitude - ctrRef(r).alt,          // 検索中心視高度差(検索中心より上=正)
                centerMode: ctx.searchCenterMode,                  // この検索で使った検索中心オプション(point/line)
                mwOffAngle: ctx.mwOffAngle,   // この検索で使ったオフセット中心角
...
/** 任意のパス start→end の統一可視判定 (標高グラフの可視判定・辻検索/My辻検索の標高フィルタ用)。
...
/** 既定のセット(ID:0)の行情報を返す */
function mySetHomeObj() {
    appState.mySetHome.id = 0;
    return appState.mySetHome;
}

/** ヘッダ集計・一括更新などが対象とする全行 (既定のセット+Myセット) */
function allMySetRows() {
    return [mySetHomeObj(), ...appState.mySets];
}

/** 保存/読込対象となるセットの内容を返す (表示中=ライブの内容、既定=退避領域、その他=スナップショット) */
function mySetDataOf(s) {
    if (s.id === appState.mySetCurrentId) return snapshotMySetData();
    return (s.id === 0 ? appState.mySetHomeData : s.data) || {};
}

/** 読み込んだ内容をセットへ反映する (保持ルール込み) */
function mySetSetData(s, data) {
    if (s.id === appState.mySetCurrentId) applyMySetData(data);
    else if (s.id === 0) appState.mySetHomeData = data;   // 既定のセットは常に端末保持
...
/** 全Myセット(既定のセット含む)のシート状態を確認して行アイコンへ反映 (ログイン直後などに実行) */
async function checkAllMySetSheets() {
    if (!isGoogleLoggedIn()) return;
    for (const s of allMySetRows()) {
...
function mySetStatusIcon() {
    if (!isGoogleLoggedIn()) return ['🈚️', 'Googleアカウントにログインします'];
    const icons = allMySetRows().map(s => mySetRowIcon(s)[0]);
...
function openMySetSheet() {
    ...
    if (id === null) return alert('開くMyセットを選択してください');
...
/** 追加ボタン: Pickerで既存のスプレッドシートを選び、選択中のMyセット(既定のセット含む)に紐付ける */
async function addMySetSheet() {
    const id = getSelectedMySetId();
    if (id === null) return alert('追加先のMyセットを選択してください');
    const s = id === 0 ? mySetHomeObj() : appState.mySets.find(x => x.id === id);
    if (!s) return;
...
function renderMySetList() {
    ...
    homeRow.innerHTML = `
        ...
        <div class="control-row">
            <label class="baseopt-radio" title="更新時に、端末の内容をGoogleドライブに保存します"><input type="radio" name="myset-sl-home" value="save" ${home.saveMode !== 'load' ? 'checked' : ''} class="myset-slmode-home">:保存</label>
            <label class="baseopt-radio" title="更新時に、Googleドライブの内容を端末に読み込みます"><input type="radio" name="myset-sl-home" value="load" ${home.saveMode === 'load' ? 'checked' : ''} class="myset-slmode-home">:読込</label>
        ...
        <div class="control-row">
            <label class="mypoint-label">メモ:</label>
            <input type="text" class="myset-memo-home" value="${escapeHtml(home.memo || '')}" placeholder="メモ(150文字)" maxlength="150" autocomplete="off">
        </div>`;
    homeRow.querySelector('.myset-check-home').addEventListener('change', (e) => { home.checked = e.target.checked; saveAppState(); });
    homeRow.querySelectorAll('.myset-slmode-home').forEach(r => r.addEventListener('change', (e) => {
        if (e.target.checked) { home.saveMode = e.target.value; saveAppState(); }
    }));
...
// 【v14(第62ラウンド)】曜日フィルタの16キーを追加(デッサン03/04)。v13と同じ「&キー名=既定値」の形で
...
function copyTsujiSearchUrl(includeDateTime) {
    ...
    params.set('tsujiDowFilter', appState.tsujiDowFilter ? 'true' : 'false');
    params.set('tsujiDowMon', appState.tsujiDowMon ? 'true' : 'false');
    params.set('tsujiDowTue', appState.tsujiDowTue ? 'true' : 'false');
    params.set('tsujiDowWed', appState.tsujiDowWed ? 'true' : 'false');
    params.set('tsujiDowThu', appState.tsujiDowThu ? 'true' : 'false');
...
function copyTsujiMeshUrl(includeDateTime) {
    ...
    params.set('tsujiMeshDowFilter', appState.tsujiMeshDowFilter ? 'true' : 'false');
    params.set('tsujiMeshDowMon', appState.tsujiMeshDowMon ? 'true' : 'false');
    params.set('tsujiMeshDowTue', appState.tsujiMeshDowTue ? 'true' : 'false');
    params.set('tsujiMeshDowWed', appState.tsujiMeshDowWed ? 'true' : 'false');
    params.set('tsujiMeshDowThu', appState.tsujiMeshDowThu ? 'true' : 'false');
...
function restoreFromUrl() {
    ...
    if (mode === 'tsujisearch') {
        ...
        if (params.has('tsujiDowFilter')) { appState.tsujiDowFilter = params.get('tsujiDowFilter') === 'true'; }
        if (params.has('tsujiDowMon')) { appState.tsujiDowMon = params.get('tsujiDowMon') === 'true'; }
        if (params.has('tsujiDowTue')) { appState.tsujiDowTue = params.get('tsujiDowTue') === 'true'; }
        if (params.has('tsujiDowWed')) { appState.tsujiDowWed = params.get('tsujiDowWed') === 'true'; }
        if (params.has('tsujiDowThu')) { appState.tsujiDowThu = params.get('tsujiDowThu') === 'true'; }
...
function setupSoramadoControls() {
    ...
    ['input-fw-api-elev', 'input-fw-ctrl-api-elev'].forEach(id => { const el = document.getElementById(id);
        if (el) el.addEventListener('change', () => {
            const v = parseFloat(el.value);
            if (!isNaN(v)) {
                if (appState.fwLat === null || appState.fwLng === null) { appState.fwLat = appState.end.lat; appState.fwLng = appState.end.lng; }
                appState.fwElev = Math.max(-500, Math.min(9000, v));
            }
            _fwShells = [];
            fwAfter();
        }); });
...
/** 打ち上げ点の緯度経度(初期値=位置情報メニューの目的点。明示設定するまで目的点に追従する) */
...
/** 打ち上げ点標高の実効値(m): 未設定の間は目的点の標高(endApiElev)を使う(初期値=目的点と同じ) */
function fwEffElev() {
    if (appState.fwLat !== null && appState.fwLng !== null) return Number(appState.fwElev) || 0;
    return Number(appState.endApiElev) || 0;
}
...
function _fwSpawnShell(now) {
    ...
    const groundTot = fwEffElev() + (Number(appState.fwHeight) || 0);
...
function fwSyncUI() {
    ...
    const p = fwLaunchPoint();
    const locStr = `${p.lat}, ${p.lng}`;   // 初期値(未設定)は目的点の座標を表示(デッサン06)
    set('input-fw-latlng', locStr); set('input-fw-ctrl-latlng', locStr);
    set('input-fw-api-elev', fwEffElev()); set('input-fw-ctrl-api-elev', fwEffElev());
...
const SS_PRESETS = {
    milkyway: { label: '天の川',        wL: 100, wM: 85, wH: 40, moon: 'avoid', wMoon: 80, unkai: 'avoid', wUnkai: 0,  wLp: 70, wTr: 40, obj: 'mw',    wObj: 60, hours: 'night' },
    stars:    { label: '星景(表示天体)', wL: 100, wM: 85, wH: 50, moon: 'avoid', wMoon: 50, unkai: 'avoid', wUnkai: 0,  wLp: 40, wTr: 30, obj: 'body',  wObj: 80, hours: 'night' },
    unkai:    { label: '雲海×月夜',     wL: 30,  wM: 90, wH: 50, moon: 'want',  wMoon: 60, unkai: 'want',  wUnkai: 90, wLp: 10, wTr: 20, obj: 'none',  wObj: 0,  hours: 'night' },
    pearl:    { label: 'パール(満月)',   wL: 100, wM: 90, wH: 50, moon: 'want',  wMoon: 90, unkai: 'avoid', wUnkai: 0,  wLp: 0,  wTr: 20, obj: 'moon',  wObj: 90, hours: 'night' },
    glow:     { label: '朝焼け・夕焼け', wL: 80,  wM: 40, wH: 20, moon: 'avoid', wMoon: 0,  unkai: 'avoid', wUnkai: 0,  wLp: 0,  wTr: 30, obj: 'none',  wObj: 0,  hours: 'ghbh' },
};

// --- IndexedDB キャッシュ (DB: soranotsuji-wx / store: fc / TTL 2時間。ローカル使い捨て・Drive同期対象外) ---
const SS_TTL = 2 * 3600e3;
let _ssDbReady = null;
function _ssDb() {
    if (_ssDbReady) return _ssDbReady;
    _ssDbReady = new Promise((ok, ng) => {
        try {
...
async function _ssIdb(mode, fn) {
    const db = await _ssDb();
...
/** 視界扇形の標本: 観測点直上1点+方位5本×距離3環。MSM格子(約5km)より粗くならないよう
 *  0.05°に丸めて重複除去する(実APIコールは数格子分に圧縮)。重み=距離減衰(近い雲ほど視界を塞ぐ)。
 *  観測点直上は頭上の雲が最も致命的なので最大重みにする(デッサン18) */
function ssFanSamples(fanDeg, rangeKm, oLatIn, oLngIn, eLatIn, eLngIn) {
    const oLat = oLatIn !== undefined ? oLatIn : appState.start.lat;
    const oLng = oLngIn !== undefined ? oLngIn : appState.start.lng;
    const eLat = eLatIn !== undefined ? eLatIn : appState.end.lat;
    const eLng = eLngIn !== undefined ? eLngIn : appState.end.lng;
    const az0 = calculateBearing(oLat, oLng, eLat, eLng);
    const raw = [{ lat: oLat, lng: oLng, w: 1.2 }];
    for (const fA of [-1, -0.5, 0, 0.5, 1]) {
        const az = (az0 + fA * fanDeg / 2) * Math.PI / 180;
        for (const fR of [0.25, 0.6, 1.0]) {
            const d = rangeKm * 1000 * fR;
            const lat = oLat + d * Math.cos(az) / 111320;
            const lng = oLng + d * Math.sin(az) / (111320 * Math.cos(oLat * Math.PI / 180));
            const w = (1 / (1 + fR)) * (Math.abs(fA) === 1 ? 0.7 : 1.0);   // 距離減衰×方位の中心寄り
            raw.push({ lat, lng, w });
        }
    }
    const byKey = new Map();
    for (const p of raw) {
        const gl = Math.round(p.lat / 0.05) * 0.05, gn = Math.round(p.lng / 0.05) * 0.05;
        const key = `${gl.toFixed(2)},${gn.toFixed(2)}`;
        const cur = byKey.get(key);
        if (!cur) byKey.set(key, { key, lat: +gl.toFixed(2), lng: +gn.toFixed(2), w: p.w });
        else cur.w = Math.max(cur.w, p.w);   // 同一格子に落ちた標本は最大重みを代表にする
    }
    return [...byKey.values()];
}

/** Open-Meteoの時刻別3層雲量+地上湿度を、キャッシュ優先+未取得分のみ複数地点バッチで取得する。
 *  days≦11は /v1/jma (気象庁GSM+MSM)。12〜16日は /v1/forecast (best_match) で延長(信頼度低)。
 *  戻り値: {key → {time:[ISO..], low:[..], mid:[..], high:[..], rh:[..]}} */
async function ssFetchClouds(points, days) {
    const d1 = Math.min(11, days);
    const results = {};
    const misses = [];
    for (const p of points) {
        let hit = null;
        try { hit = await _ssIdb('readonly', s => s.get(`jma:${p.key}:${days}`)); } catch (_) {}
        if (hit && Date.now() - hit.t < SS_TTL) results[p.key] = hit.data;
        else misses.push(p);
    }
    if (misses.length) {
        const lats = misses.map(p => p.lat).join(','), lngs = misses.map(p => p.lng).join(',');
        const H = 'cloud_cover_low,cloud_cover_mid,cloud_cover_high,relative_humidity_2m';
        const fetchArr = async (base, fd) => {
            const url = `${base}?latitude=${lats}&longitude=${lngs}&hourly=${H}&forecast_days=${fd}&timezone=Asia%2FTokyo`;
            const res = await fetch(url);
            if (!res.ok) throw new Error(`Open-Meteo HTTP ${res.status}`);
            const data = await res.json();
            return Array.isArray(data) ? data : [data];
        };
        const arr = await fetchArr('https://api.open-meteo.com/v1/jma', d1);
        let ext = null;
        if (days > 11) {
            try { ext = await fetchArr('https://api.open-meteo.com/v1/forecast', Math.min(16, days)); } catch (_) { ext = null; }
        }
        misses.forEach((p, i) => {
            const h = arr[i] && arr[i].hourly;
            if (!h) return;
            const data = { time: h.time, low: h.cloud_cover_low, mid: h.cloud_cover_mid, high: h.cloud_cover_high, rh: h.relative_humidity_2m };
            // 12日目以降は best_match の延長分を継ぎ足す(同時刻はJMA優先)
            const eh = ext && ext[i] && ext[i].hourly;
            if (eh) for (let k = 0; k < eh.time.length; k++) {
                if (eh.time[k] > data.time[data.time.length - 1]) {
                    data.time.push(eh.time[k]); data.low.push(eh.cloud_cover_low[k]); data.mid.push(eh.cloud_cover_mid[k]);
                    data.high.push(eh.cloud_cover_high[k]); data.rh.push(eh.relative_humidity_2m[k]);
                }
            }
            results[p.key] = data;
            _ssIdb('readwrite', s => s.put({ t: Date.now(), data }, `jma:${p.key}:${days}`)).catch(() => {});
        });
    }
    return results;
}

/** AOD(エアロゾル光学的厚さ)を観測点格子1点だけ取得する(空間変化が緩やかなため)。
 *  air-quality APIの予報は7日上限。失敗・範囲外はnull(透明度は湿度のみの従来動作)。
 *  戻り値: {timeIdx: Map(ISO→idx), aod:[..]} | null */
async function ssFetchAod(point, days) {
    const fd = Math.min(7, days);
    const key = `aq:${point.key}:${fd}`;
    try {
        const hit = await _ssIdb('readonly', s => s.get(key)).catch(() => null);
        let data = (hit && Date.now() - hit.t < SS_TTL && hit.data && hit.data.aod) ? hit.data : null;
        if (!data) {
            const url = `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${point.lat}&longitude=${point.lng}&hourly=aerosol_optical_depth&forecast_days=${fd}&timezone=Asia%2FTokyo`;
            const res = await fetch(url);
            if (!res.ok) throw new Error(`AOD HTTP ${res.status}`);
            const j = await res.json();
            if (!j.hourly || !j.hourly.time || !j.hourly.aerosol_optical_depth) throw new Error('AOD形式');
            data = { time: j.hourly.time, aod: j.hourly.aerosol_optical_depth };
            _ssIdb('readwrite', s => s.put({ t: Date.now(), data }, key)).catch(() => {});
        }
        const timeIdx = new Map();
        data.time.forEach((t, i) => timeIdx.set(t, i));
        return { timeIdx, aod: data.aod };
    } catch (_) { return null; }
}

// 雲海判定に使う気圧面(低層)。JMAの気圧面雲量はRHからの近似(Sundqvist 1989)のため「目安」
const SS_PL_LEVELS = [1000, 975, 950, 925, 900, 850, 800, 700];
/** 気圧面の雲量×実高度(雲海判定用)+300hPa風速(シーイングの目安)を観測点格子1点だけ取得する。
 *  戻り値: {timeIdx, cc:{lev:[..]}, gh:{lev:[..]}, wind300:[..]} | null */
async function ssFetchPressure(point, days) {
    const fd = Math.min(11, days);
    const key = `pl:${point.key}:${fd}`;
    try {
        const hit = await _ssIdb('readonly', s => s.get(key)).catch(() => null);
        let data = (hit && Date.now() - hit.t < SS_TTL) ? hit.data : null;
        if (!data) {
            const H = SS_PL_LEVELS.map(l => `cloud_cover_${l}hPa,geopotential_height_${l}hPa`).join(',') + ',wind_speed_300hPa';
            const url = `https://api.open-meteo.com/v1/jma?latitude=${point.lat}&longitude=${point.lng}&hourly=${H}&forecast_days=${fd}&timezone=Asia%2FTokyo&wind_speed_unit=ms`;
            const res = await fetch(url);
            if (!res.ok) throw new Error(`PL HTTP ${res.status}`);
            const j = await res.json();
            const h = j.hourly;
            if (!h || !h.time || !h[`cloud_cover_${SS_PL_LEVELS[0]}hPa`]) throw new Error('PL形式');
            data = { time: h.time, cc: {}, gh: {}, wind300: h.wind_speed_300hPa || [] };
            for (const l of SS_PL_LEVELS) { data.cc[l] = h[`cloud_cover_${l}hPa`]; data.gh[l] = h[`geopotential_height_${l}hPa`]; }
            _ssIdb('readwrite', s => s.put({ t: Date.now(), data }, key)).catch(() => {});
        }
        const timeIdx = new Map();
        data.time.forEach((t, i) => timeIdx.set(t, i));
        return { timeIdx, cc: data.cc, gh: data.gh, wind300: data.wind300 };
    } catch (_) { return null; }
}

/** 雲海度(0〜100)の推定: 「観測点標高が低層雲の雲頂より上」×「眼下の低層雲量」(デッサン18)。
 *  雲頂=雲量60%以上の気圧面のうち最も高い実高度(3500m未満の低層のみ)。
 *  充足度=観測点が雲頂より50m以上高いことから150mで飽和する線形。気圧面雲量はRH近似のため目安。
 *  戻り値: {deg, top, below} | null(気圧面データ無し・該当時刻無し) */
function ssUnkaiHour(pl, timeStr, obsElev) {
    if (!pl) return null;
    const i = pl.timeIdx.get(timeStr);
    if (i === undefined) return null;
    let top = null, below = 0, any = false;
    for (const l of SS_PL_LEVELS) {
        const cc = pl.cc[l] && pl.cc[l][i], gh = pl.gh[l] && pl.gh[l][i];
        if (cc === null || cc === undefined || gh === null || gh === undefined) continue;
        any = true;
        if (gh < 3500 && cc >= 60 && (top === null || gh > top)) top = gh;
        if (gh < obsElev) below = Math.max(below, cc);
    }
    if (!any) return null;
    if (top === null || obsElev < top + 50) return { deg: 0, top, below };
    const fill = Math.min(1, (obsElev - top - 50) / 150 + 0.33);   // 50mで0.33、200m差で飽和
    return { deg: Math.round(below * Math.min(1, Math.max(0, fill))), top, below };
}

/** ERA5過去統計: 同月日±7日×過去10年の夜間(20時〜翌4時の目安)晴天率(全雲量30%未満の割合)。
 *  年毎にIndexedDBへキャッシュ(TTL7日)。戻り値 {rate, years, hours} | null */
async function ssFetchEra5Stat(point, setBusy) {
    const now = new Date();
    const mmdd = `${('0' + (now.getMonth() + 1)).slice(-2)}${('0' + now.getDate()).slice(-2)}`;
    let clearH = 0, nightH = 0, years = 0;
    for (let k = 1; k <= 10; k++) {
        const y = now.getFullYear() - k;
        const key = `era5:${point.key}:${mmdd}:${y}`;
        let data = null;
        try {
            const hit = await _ssIdb('readonly', s => s.get(key)).catch(() => null);
            if (hit && Date.now() - hit.t < 7 * 86400e3) data = hit.data;
            if (!data) {
                if (setBusy) setBusy(`過去統計を取得中… (${y}年)`);
                const c = new Date(y, now.getMonth(), now.getDate());
                const d0 = new Date(c.getTime() - 7 * 86400e3), d1 = new Date(c.getTime() + 7 * 86400e3);
                const f = d => `${d.getFullYear()}-${('0' + (d.getMonth() + 1)).slice(-2)}-${('0' + d.getDate()).slice(-2)}`;
                const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${point.lat}&longitude=${point.lng}&start_date=${f(d0)}&end_date=${f(d1)}&hourly=cloud_cover&timezone=Asia%2FTokyo`;
                const res = await fetch(url);
                if (!res.ok) throw new Error(`ERA5 HTTP ${res.status}`);
                const j = await res.json();
                if (!j.hourly || !j.hourly.time) throw new Error('ERA5形式');
                data = { time: j.hourly.time, cc: j.hourly.cloud_cover };
                _ssIdb('readwrite', s => s.put({ t: Date.now(), data }, key)).catch(() => {});
            }
        } catch (_) { continue; }
        years++;
        for (let i = 0; i < data.time.length; i++) {
            const hh = parseInt(data.time[i].slice(11, 13));
            if (!(hh >= 20 || hh < 4)) continue;   // 夜間の目安(20時〜翌4時)
            const cc = data.cc[i];
    ...
    if (!nightH) return null;
    return { rate: Math.round(clearH / nightH * 1000) / 10, years, hours: nightH };
}
...
function ssAggregateHour(points, clouds, hourIdx) {
    ...
    for (const p of points) {
        const c = clouds[p.key];
        if (!c || c.low[hourIdx] === undefined || c.low[hourIdx] === null) continue;
        sw += p.w;
        low += p.w * c.low[hourIdx]; mid += p.w * c.mid[hourIdx]; high += p.w * c.high[hourIdx]; rh += p.w * (c.rh[hourIdx] || 0);
    }
    if (!sw) return null;
 ...
 *  oPos(省略可)={lat,lng,elev}: 観測点の上書き(My宙検索の一括用。省略時は現在の観測点) */
function ssAstroHour(date, preset, oPos) {
    const o = oPos || appState.start;
    const obs = new Astronomy.Observer(o.lat, o.lng, o.elev || 0);
    ...
    if (preset.obj === 'mw') {
        ...
        objAlt = Astronomy.Horizon(date, obs, gc.ra, gc.dec, 'normal').altitude;
    } else if (preset.obj === 'moon') {
        objAlt = moonH.altitude;
    } else if (preset.obj === 'body') {
        // 表示天体の中で最も高いもの(フェーズ1の簡易版)
        let best = null;
        for (const b of appState.bodies.filter(b => b.visible)) {
            try {
                const h = isFixedStar(b.id)
                    ? (() => { const rd = getFixedStarRaDec(b.id); return Astronomy.Horizon(date, obs, rd.ra, rd.dec, 'normal'); })()
                    : hor(b.id);
                if (best === null || h.altitude > best) best = h.altitude;
            } catch (_) {}
        }
        objAlt = best;
    }
 ...
 *  雲(3層)×月×対象高度×透明度(湿度+AOD)×光害(lp={zen,dir}のSQM。省略時はスキップ)×雲海(狙う時)。
 *  env(省略可)={aodNorm:0〜1|null, unkai:0〜100|null}。戻り値 {clear,fMoon,fObj,fTr,fLp,fUnkai,humid,haze,score} */
function ssScoreParts(preset, agg, astro, lp, env) {
    const clear = (1 - preset.wL / 100 * agg.low / 100) * (1 - preset.wM / 100 * agg.mid / 100) * (1 - preset.wH / 100 * agg.high / 100);
    let fMoon = 1;
    if (preset.wMoon > 0) {
        if (preset.moon === 'avoid') {
            const moonUp = Math.max(0, Math.sin(astro.moonAlt * Math.PI / 180)) * astro.moonIllum;   // 月明かりの強さ0〜1
            fMoon = 1 - preset.wMoon / 100 * moonUp;
        } else {
            const seen = astro.moonAlt > 0 ? astro.moonIllum : 0;   // 月が出ていて明るいほど良い
            fMoon = (1 - preset.wMoon / 100) + preset.wMoon / 100 * seen;
        }
    }
    let fObj = 1;
    if (preset.wObj > 0 && astro.objAlt !== null) {
        const g = astro.objAlt <= 0 ? 0 : Math.min(1, astro.objAlt / 20);   // 高度20°以上で満点・地平線下で0
        fObj = (1 - preset.wObj / 100) + preset.wObj / 100 * g;
    }
    const humid = Math.max(0, Math.min(1, ((agg.rh || 0) - 60) / 40));      // 湿度60%以下は透明・100%で最大減点
    // 霞み度0〜1: 湿度とAOD(0.1=澄む〜0.5=霞む)の悪い方(どちらかが悪ければ空は濁る)。AOD無しは湿度のみ
    const aodNorm = env && env.aodNorm !== null && env.aodNorm !== undefined ? env.aodNorm : null;
    const haze = aodNorm !== null ? Math.max(humid, aodNorm) : humid;
    const fTr = 1 - preset.wTr / 100 * 0.5 * haze;
    let fUnkai = 1;
    if (preset.unkai === 'want' && preset.wUnkai > 0 && env && env.unkai !== null && env.unkai !== undefined) {
        fUnkai = (1 - preset.wUnkai / 100) + preset.wUnkai / 100 * env.unkai / 100;   // 雲海度が高いほど加点
    }
    let fLp = 1;
    ...
    const score = Math.max(0, Math.min(100, Math.round(1000 * clear * fMoon * fObj * fTr * fLp * fUnkai) / 10));
    return { clear, fMoon, fObj, fTr, fLp, fUnkai, humid, haze, score };
}
...
async function ssRunSearchCore(cond, setBusy) {
    const { oLat, oLng, oElev, eLat, eLng, preset, days, stepH, fanDeg, bands } = cond;
    const distKm = getDistanceWGS84(oLat, oLng, eLat, eLng) / 1000;
    const rangeKm = cond.rangeKm !== null && cond.rangeKm !== undefined ? cond.rangeKm : Math.max(1, Math.min(300, Math.ceil(distKm)));
    setBusy('扇形標本を作成中…');
    const points = ssFanSamples(fanDeg, rangeKm, oLat, oLng, eLat, eLng);
    // 光害(静的アセット): 観測点の天頂SQMと、扇形に沿った方向光害。検索毎に1回だけ算出
    await ssLoadLp();
    const lp = { zen: ssLpSqm(oLat, oLng), dir: ssLpDirectional(points) };
    ...
    // フェーズ3: AOD(透明度)+気圧面(雲海度・上空風)は観測点格子1点。失敗しても検索は続行(該当因子はスキップ)
    setBusy('AOD/気圧面を取得中…');
    const [aq, pl] = await Promise.all([ssFetchAod(points[0], days), ssFetchPressure(points[0], days)]);
    setBusy('宙スコアを計算中…');
    const rows = [];
    const now = Date.now();
    const oPos = { lat: oLat, lng: oLng, elev: oElev || 0 };
    for (let i = 0; i < base.time.length; i += stepH) {
        const timeStr = base.time[i];
        const dt = new Date(timeStr);
        if (isNaN(dt.getTime()) || dt.getTime() < now - 3600e3) continue;
        const agg = ssAggregateHour(points, clouds, i);
        if (!agg) continue;
        const astro = ssAstroHour(dt, preset, oPos);
        ...
        const aodV = aq && aq.timeIdx.has(timeStr) ? aq.aod[aq.timeIdx.get(timeStr)] : null;
        const aodNorm = aodV === null || aodV === undefined ? null : Math.max(0, Math.min(1, (aodV - 0.1) / 0.4));
        const uk = ssUnkaiHour(pl, timeStr, oPos.elev);
        const wind300 = pl && pl.timeIdx.has(timeStr) ? pl.wind300[pl.timeIdx.get(timeStr)] : null;
        const env = { aodNorm, unkai: uk ? uk.deg : null };
        const parts = ssScoreParts(preset, agg, astro, lp, env);
        rows.push({ dt, hourIdx: i, timeStr, band, conf, agg, astro, env, aodV, uk, wind300, parts, score: parts.score });
        if (rows.length % 200 === 0) await new Promise(r => setTimeout(r, 0));
    }
    const az0 = calculateBearing(oLat, oLng, eLat, eLng);
    // 観測点も保存する(検索後に位置情報を動かしても、詳細/地図は検索時の観測点基準で一貫させる)
    const snap = { rows, points, clouds, lp, aq, pl, preset, days, stepH, fanDeg, rangeKm, az0, oLat, oLng, oElev: oPos.elev, bands };
    rows.forEach(r => { r._snap = snap; });
    // 統計行: ERA5の同時期(同月日±7日×過去10年)の夜間晴天率を末尾に1行追加(取得失敗時はスキップ)
    if (cond.stat) {
        const st = await ssFetchEra5Stat(points[0], setBusy);
        if (st) {
            rows.push({
                isStat: true, stat: st, _snap: snap, score: st.rate,
                dt: new Date(8640000000000000), band: 'night', conf: '統計',
                agg: { low: -1, mid: -1, high: -1, rh: -1 },
                astro: { sunAlt: -99, moonAlt: -999, moonIllum: -1, moonAge: -1, objAlt: null },
                env: { aodNorm: null, unkai: null },
                parts: { clear: -1, fMoon: 1, fObj: 1, fTr: 1, fLp: 1, fUnkai: 1, humid: -1, haze: -1, score: st.rate },
            });
        }
    }
    return snap;
...
/** 宙検索の本体(単発): フォーム→appState反映 → 現在の観測点×目的点で実行。スナップショットを返す */
async function ssExecuteSearch(setBusy) {
    ...
    const statChk = document.getElementById('chk-ss-stat');
    if (statChk) appState.ssStat = statChk.checked;
    const cond = {
        oLat: appState.start.lat, oLng: appState.start.lng, oElev: appState.start.elev,
        eLat: appState.end.lat, eLng: appState.end.lng,
        preset: ssEffectivePreset(), days, stepH, fanDeg, rangeKm: appState.ssRange,
        bands: { night: appState.ssBandNight, twilight: appState.ssBandTwilight, ghbh: appState.ssBandGhbh, day: appState.ssBandDay },
        stat: appState.ssStat,
    };
    const snap = await ssRunSearchCore(cond, setBusy);
    _ssLast = snap;
    _ssSelRow = null;
    return snap;
}
...
async function soraSearchRun() {
    ...
    try {
        ...
        ssRenderResults({ rows: snap.rows, title: `宙検索結果 (${snap.preset.label})`, batch: false });
 ...
 *  統計行(r.isStat)は「同時期±7日×過去10年の夜間晴天率」の特別表記 */
function ssRowValues(r) {
    const p2 = v => ('00' + v).slice(-2);
    const dows = ['日', '月', '火', '水', '木', '金', '土'];
    const lp = r._snap.lp;
    if (r.isStat) {
        const now = new Date();
        return [
            `${p2(now.getMonth() + 1)}/${p2(now.getDate())}±7日`, '-', '-', '夜',
            r.stat.rate.toFixed(1), r.stat.rate.toFixed(1),
            '-', '-', '-', '-', '-', '-', '-', '-',
    ...
    return [
        ...
        r.env && r.env.unkai !== null && r.env.unkai !== undefined ? `${r.env.unkai}%` : '-',   // 雲海度(気圧面から推定。目安)
        `${Math.round(100 - r.parts.haze * 100)}%`,            // 透明度の目安(湿度+AODの悪い方。100%=澄む)
 ...
 *  view = { rows, title, batch } : batch=trueで先頭に宙検索ID/宙検索名の2列を追加(My宙検索の一括) */
function ssRenderResults(view) {
    ...
    const renderRow = (r) => {
        ...
        const v = ssRowValues(r);
        const scoreColor = r.score >= 70 ? '#ffd700' : (r.score >= 40 ? '#eee' : '#888');
        const confColor = r.conf === '高' ? '#8f8' : (r.conf === '中' ? '#ff8' : (r.conf === '統計' ? '#c9f' : '#f88'));
        const head = view.batch ? `<td>${r._entry ? r._entry.id : '-'}</td><td>${escapeHtml(r._entry ? (r._entry.name || '') : '')}</td>` : '';
        tr.innerHTML = head + `<td>${v[0]}</td><td>${r.isStat ? '-' : '(' + v[1] + ')'}</td><td>${v[2]}</td><td>${escapeHtml(v[3])}</td>` +
            ...
            `<td style="color:${confColor};">${escapeHtml(v[17])}</td>`;
        tr.addEventListener('click', () => {
            if (r.isStat) return;   // 統計行は特定の日時を持たないため選択しない
            ...
            _ssSelRow = r;
            ssShowDetail(r._snap, r);
            ssUpdateMapOverlay(r._snap, r);
        });
        return tr;
    };
    const table = document.createElement('table');
    table.className = 'td-table';
    const headers = view.batch ? ['宙検索ID', '宙検索名'].concat(SS_LIST_HEADERS) : SS_LIST_HEADERS;
    const thead = document.createElement('thead');
    ...
    const columns = [
        ...
        { label: '雲海度', compare: (a, b) => ((a.env && a.env.unkai) ?? -1) - ((b.env && b.env.unkai) ?? -1) },
        { label: '透明度', compare: (a, b) => b.parts.haze - a.parts.haze },
        ...
        { label: '光害(SQM)', compare: (a, b) => ((a._snap.lp && a._snap.lp.zen) ?? 0) - ((b._snap.lp && b._snap.lp.zen) ?? 0) },
        { label: '方向光害', compare: (a, b) => ((a._snap.lp && a._snap.lp.dir) ?? 0) - ((b._snap.lp && b._snap.lp.dir) ?? 0) },
        { label: '対象高度', compare: (a, b) => (a.astro.objAlt ?? -999) - (b.astro.objAlt ?? -999) },
        { label: '信頼度', compare: (a, b) => confRank[a.conf] - confRank[b.conf] },
    ];
    if (view.batch) {
        columns.unshift(
            { label: '宙検索ID', compare: (a, b) => (a._entry ? a._entry.id : 0) - (b._entry ? b._entry.id : 0) },
            { label: '宙検索名', compare: (a, b) => String(a._entry ? a._entry.name : '').localeCompare(String(b._entry ? b._entry.name : '')) });
    }
    setupTableSort(table, rows, columns, renderRow, []);
}
...
function ssShowDetail(snap, r) {
    ...
    // 因子別の加減点(乗算型の係数)。宙スコア = 100 × 晴天度 × 月 × 対象 × 透明度 × 光害 × 雲海
    const aodStr = r.aodV !== null && r.aodV !== undefined ? `AOD ${r.aodV.toFixed(2)}` : 'AODデータなし(7日超/取得不可)';
    const ukStr = r.uk === null ? '気圧面データなし'
        : `雲海度${r.env.unkai}% (雲頂${r.uk.top !== null ? Math.round(r.uk.top) + 'm' : 'なし'} / 観測点${Math.round(snap.oElev)}m / 眼下雲量${Math.round(r.uk.below)}%)`;
    const factorRows = [
        ...
        ['透明度', `湿度${Math.round(r.agg.rh)}% / ${aodStr} → 霞み${Math.round(parts.haze * 100)}% (重み${preset.wTr})`, f(parts.fTr)],
        ...
        ['雲海', `${preset.unkai === 'want' ? '狙う' : '避ける(低層雲の扱いに含む)'}(重み${preset.wUnkai}) ${ukStr}`, preset.unkai === 'want' && preset.wUnkai > 0 && r.env.unkai !== null ? f(parts.fUnkai) : '×1.00'],
    ];
    ...
    html += '</tbody></table>';
    // 気圧面プロファイル(雲海判定の根拠)+上空風(シーイングの目安)
    const pl = snap.pl;
    const pi = pl && r.timeStr !== undefined ? pl.timeIdx.get(r.timeStr) : undefined;
    if (pl && pi !== undefined) {
        html += '<table class="td-table"><thead><tr><th>気圧面</th>' + SS_PL_LEVELS.map(l => `<th>${l}hPa</th>`).join('') + '</tr></thead><tbody>';
        html += '<tr><td>実高度</td>' + SS_PL_LEVELS.map(l => { const v = pl.gh[l] && pl.gh[l][pi]; return `<td>${v !== null && v !== undefined ? Math.round(v) + 'm' : '-'}</td>`; }).join('') + '</tr>';
        html += '<tr><td>雲量</td>' + SS_PL_LEVELS.map(l => { const v = pl.cc[l] && pl.cc[l][pi]; return `<td>${v !== null && v !== undefined ? Math.round(v) + '%' : '-'}</td>`; }).join('') + '</tr>';
        html += '</tbody></table>';
        const w300 = r.wind300;
        html += `<div class="ss-detail-note">上空風(300hPa): ${w300 !== null && w300 !== undefined ? w300.toFixed(1) + ' m/s' : '-'} — シーイングの目安(ジェット気流直下=約25m/s以上で星像が揺らぎやすい)。気圧面の雲量はRHからの近似(Sundqvist 1989)のため雲海度は目安です。</div>`;
    } else {
        html += '<div class="ss-detail-note">気圧面プロファイル: この時刻のデータはありません(気圧面は11日先まで)。</div>';
    }
    html += '<div class="ss-detail-note">標本の雲量はMSM/GSM格子(約5km)の値、寄与重みは距離減衰×方位の中心寄り(直上=最大1.2)です。透明度はAOD(エアロゾル)と湿度の悪い方から算出しています(AODは7日先まで)。</div>';
    body.innerHTML = html;
...
async function fileSoraSearchCsv() {
    if (_ssFileRunning || _ssRunning || _mySoraBatchRunning) return;
...
async function ssDownloadCsv(snap) {
    ...
    const bandsStr = snap.bands
        ? Object.keys(SS_BAND_LABELS).filter(k => snap.bands[k]).map(k => SS_BAND_LABELS[k]).join('/')
        : '';
    const fixed = [
...
function syncSoraSearchUI() {
    ...
    chk('chk-ss-stat', appState.ssStat);
}
...
function setupSoraSearchControls() {
    ...
    [['chk-ss-band-night', 'ssBandNight'], ['chk-ss-band-twilight', 'ssBandTwilight'], ['chk-ss-band-ghbh', 'ssBandGhbh'], ['chk-ss-band-day', 'ssBandDay'], ['chk-ss-stat', 'ssStat']].forEach(([id, key]) => {
        const el = document.getElementById(id);
...
// ============================================================
// My宙検索 (フェーズ4。メニュー分割ポリシー: 宙検索=現在の1組 / My宙検索=My観測点×My目的点の複数組の一括)
//  エントリは観測点ID/目的点IDでMyリストを参照し、狙い(プリセット or カスタム重み一式)と検索条件を保持する。
//  Myセット(シート同期)・セット切り替えの対象には含めていない(対象化はシート構成の判断が必要なため)。
// ============================================================
let mySoraDirty = false;
let _mySoraBatchRunning = false, _mySoraBatchCancel = false;
...
function getSelectedMySoraId() {
    const radio = document.querySelector('input[name="mysora-select"]:checked');
    return radio ? parseInt(radio.value) : null;
}
function getNextMySoraId() {
    for (let i = 1; i <= 1000; i++) if (!appState.mySoraSearches.some(t => t.id === i)) return i;
    alert('My宙検索の登録上限(1000件)に達しています');
    return null;
}
/** 現在の宙検索メニューのカスタム重み一式(+時間帯)のスナップショット */
function mySoraWeightsFromCurrent() {
    return {
        wL: appState.ssWL, wM: appState.ssWM, wH: appState.ssWH,
        moon: appState.ssMoonMode, wMoon: appState.ssWMoon,
        unkai: appState.ssUnkaiMode, wUnkai: appState.ssWUnkai,
        wLp: appState.ssWLp, wTr: appState.ssWTr, obj: appState.ssObj, wObj: appState.ssWObj,
        bands: { night: appState.ssBandNight, twilight: appState.ssBandTwilight, ghbh: appState.ssBandGhbh, day: appState.ssBandDay },
    };
}
/** エントリ→検索条件1組(ssRunSearchCore用)。obs/tgtはMy観測点/My目的点の実体 */
function mySoraEntryCond(t, obs, tgt) {
    let preset, bands;
    if (t.preset === 'custom' && t.weights) {
        const w = t.weights;
        preset = { key: 'custom', label: 'カスタム', wL: w.wL, wM: w.wM, wH: w.wH, moon: w.moon, wMoon: w.wMoon, unkai: w.unkai, wUnkai: w.wUnkai, wLp: w.wLp, wTr: w.wTr, obj: w.obj, wObj: w.wObj };
        bands = w.bands || { night: true, twilight: false, ghbh: false, day: false };
    } else {
        const p = SS_PRESETS[t.preset] || SS_PRESETS.milkyway;
        preset = { key: t.preset, label: p.label, ...p };
        bands = { night: p.hours === 'night', twilight: false, ghbh: p.hours === 'ghbh', day: false };
    }
    return {
        oLat: obs.lat, oLng: obs.lng, oElev: (obs.elev !== null && obs.elev !== undefined ? obs.elev : 0) + (Number(obs.height) || 0),
        eLat: tgt.lat, eLng: tgt.lng,
        preset, days: t.days, stepH: t.interval, fanDeg: t.fan, rangeKm: t.range,
        bands, stat: false,
    };
}

/** 宙検索取得: 現在の観測点/目的点と宙検索メニューの条件を1件のMy宙検索として追加 */
function getMySoraFromSoraSearch() {
    if (appState.mySoraSearches.length >= 1000) return alert('My宙検索の登録上限(1000件)に達しています');
    if (!confirm('現在の観測点/目的点の位置情報(緯度経度・標高・高さ)と、宙検索メニューの条件(狙い・重み・期間等)を、My宙検索リストに追加しますか？')) return;
    const id = getNextMySoraId();
    if (id === null) return;
    const obsId = findOrCreateMyObsFromCurrent();
    if (obsId === null) return;
    const tgtId = findOrCreateMyTgtFromCurrent();
    if (tgtId === null) return;
    appState.mySoraSearches.push({
        id, name: '新規宙検索名', obsId, tgtId,
        preset: appState.ssPreset,
        weights: appState.ssPreset === 'custom' ? mySoraWeightsFromCurrent() : null,
        days: appState.ssDays, interval: appState.ssInterval, fan: appState.ssFan, range: appState.ssRange,
        checked: false, memo: '',
    });
    saveAppState();
    setMySoraDirty(true);
    renderMySoraSearches();
    const radio = document.querySelector(`input[name="mysora-select"][value="${id}"]`);
    if (radio) radio.checked = true;
}

/** 行追加(空のMy宙検索) */
function addMySoraRow() {
    if (appState.mySoraSearches.length >= 1000) return alert('My宙検索の登録上限(1000件)に達しています');
    if (!confirm('My宙検索リストの末尾に宙検索の行を追加しますか？')) return;
    const id = getNextMySoraId();
    if (id === null) return;
    appState.mySoraSearches.push({
        id, name: '', obsId: null, tgtId: null,
        preset: 'milkyway', weights: null,
        days: 11, interval: 1, fan: 24, range: null,
        checked: false, memo: '',
    });
    saveAppState();
    setMySoraDirty(true);
...
function deleteMySoraRow() {
    const id = getSelectedMySoraId();
    if (id === null) return alert('削除するMy宙検索を選択してください');
    const t = appState.mySoraSearches.find(x => x.id === id);
    if (!t) return;
    if (!confirm(`My宙検索リストの宙検索（ID:${id}、${t.name || ''}）を削除しますか？`)) return;
    appState.mySoraSearches = appState.mySoraSearches.filter(x => x.id !== id);
    saveAppState();
    setMySoraDirty(true);
    renderMySoraSearches();
}
function moveMySora(delta) {
    const id = getSelectedMySoraId();
    if (id === null) return;
...
/** 全て登録: 未入力の検証 + dirtyクリア */
function registerAllMySora() {
    const errEl = document.getElementById('mysora-error');
    for (const t of appState.mySoraSearches) {
        if (!t.name || t.obsId == null || t.tgtId == null) {
            if (errEl) errEl.innerHTML = `<span class="mypoint-error-text">宙検索ID:${t.id}に未入力のものがあります。入力するか、行削除してください。</span>`;
            return;
        }
        if (!appState.myObservations.some(o => o.id === t.obsId)) {
            if (errEl) errEl.innerHTML = `<span class="mypoint-error-text">宙検索ID:${t.id}の観測点ID:${t.obsId}はMy観測点リストに存在しません。</span>`;
            return;
        }
        if (!appState.myTargets.some(g => g.id === t.tgtId)) {
            if (errEl) errEl.innerHTML = `<span class="mypoint-error-text">宙検索ID:${t.id}の目的点ID:${t.tgtId}はMy目的点リストに存在しません。</span>`;
            return;
        }
    }
...
/** リスト描画(My辻検索と同じ操作感の簡易版) */
function renderMySoraSearches() {
    const container = document.getElementById('mysora-list');
    if (!container) return;
    container.innerHTML = '';
    const list = appState.mySoraSearches;
    if (!list.length) {
        container.innerHTML = '<div class="mystars-empty">My宙検索は登録されていません</div>';
        return;
    }
    const presetOpts = Object.entries(SS_PRESETS).map(([k, p]) => [k, p.label]).concat([['custom', 'カスタム']]);
    list.forEach((t, idx) => {
        const row = document.createElement('div');
        row.className = 'mypoint-row';
        row.innerHTML = `
            <hr class="tsujisearch-separator">
            <div class="mypoint-row-header">
                <input type="radio" name="mysora-select" value="${t.id}" class="mystars-radio" ${idx === 0 ? 'checked' : ''}>
                <input type="checkbox" class="body-checkbox mysora-check" data-id="${t.id}" ${t.checked ? 'checked' : ''}>
                <span class="mypoint-id">ID:${String(t.id).padStart(4, ' ')}</span>
            </div>
            <div class="control-row">
                <input type="text" class="mysora-name" value="${escapeHtml(t.name || '')}" placeholder="宙検索名" maxlength="150" data-id="${t.id}" autocomplete="off">
            </div>
            <div class="control-row">
                <label class="mytsuji-label">観測点ID:</label>
                <input type="number" class="mysora-obsid" value="${t.obsId ?? ''}" placeholder="観測点ID(数字)" step="1" min="1" max="1000" data-id="${t.id}">
                <label class="mytsuji-label">目的点ID:</label>
                <input type="number" class="mysora-tgtid" value="${t.tgtId ?? ''}" placeholder="目的点ID(数字)" step="1" min="1" max="1000" data-id="${t.id}">
            </div>
            <div class="control-row">
                <label class="mytsuji-label">狙い:</label>
                <select class="mysora-preset" data-id="${t.id}" title="評価尺度。カスタムはこの行に保存された重み一式(登録/変更時点の宙検索メニューの値)を使います">
                    ${presetOpts.map(([k, l]) => `<option value="${k}" ${t.preset === k ? 'selected' : ''}>${l}</option>`).join('')}
                </select>
            </div>
            <div class="control-row">
                <label class="mytsuji-label">検索期間(日):</label>
                <input type="number" class="mysora-days" value="${t.days}" step="1" min="1" max="16" data-id="${t.id}">
                <label class="mytsuji-label">間隔:</label>
                <select class="mysora-interval" data-id="${t.id}">
                    <option value="1" ${t.interval !== 3 ? 'selected' : ''}>1時間</option>
                    <option value="3" ${t.interval === 3 ? 'selected' : ''}>3時間</option>
                </select>
            </div>
            <div class="control-row">
                <label class="mytsuji-label">扇角(°):</label>
                <input type="number" class="mysora-fan" value="${t.fan}" step="1" min="1" max="180" data-id="${t.id}">
                <label class="mytsuji-label">範囲(km):</label>
                <input type="number" class="mysora-range" value="${t.range ?? ''}" placeholder="自動" step="1" min="1" max="300" data-id="${t.id}">
            </div>
            <div class="control-row">
                <input type="text" class="mysora-memo" value="${escapeHtml(t.memo || '')}" placeholder="メモ" maxlength="150" data-id="${t.id}" autocomplete="off">
            </div>`;
        container.appendChild(row);
    });
    const upd = (id, fn) => {
    ...
    container.querySelectorAll('.mysora-check').forEach(el => el.addEventListener('change', e => upd(parseInt(el.dataset.id), t => { t.checked = e.target.checked; })));
    container.querySelectorAll('.mysora-name').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => { t.name = el.value; })));
    container.querySelectorAll('.mysora-memo').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => { t.memo = el.value; })));
    container.querySelectorAll('.mysora-obsid').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => { const v = parseInt(el.value); t.obsId = isNaN(v) ? null : v; })));
    container.querySelectorAll('.mysora-tgtid').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => { const v = parseInt(el.value); t.tgtId = isNaN(v) ? null : v; })));
    container.querySelectorAll('.mysora-preset').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => {
        t.preset = el.value;
        // カスタムへ変更した時点の宙検索メニューの重み一式をこの行へ取り込む(以後は行に固定)
        t.weights = el.value === 'custom' ? (t.weights || mySoraWeightsFromCurrent()) : null;
    })));
    container.querySelectorAll('.mysora-days').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => { t.days = Math.max(1, Math.min(16, parseInt(el.value) || 11)); el.value = t.days; })));
    container.querySelectorAll('.mysora-interval').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => { t.interval = parseInt(el.value) === 3 ? 3 : 1; })));
    container.querySelectorAll('.mysora-fan').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => { t.fan = Math.max(1, Math.min(180, parseFloat(el.value) || 24)); el.value = t.fan; })));
    container.querySelectorAll('.mysora-range').forEach(el => el.addEventListener('change', () => upd(parseInt(el.dataset.id), t => {
        const v = parseFloat(el.value);
        t.range = isFinite(v) && v > 0 ? Math.max(1, Math.min(300, v)) : null;
        el.value = t.range ?? '';
    })));
}

/** 一括計算/File取得(fileMode=true)。実行中の再押下はキャンセル。結果は宙検索結果パネル(宙検索ID/名の2列付き) */
async function runMySoraBatch(fileMode) {
    if (_mySoraBatchRunning) { _mySoraBatchCancel = true; return; }   // 実行中の再押下=キャンセル
    if (_ssRunning || _ssFileRunning) return;   // 単発の宙検索/File取得の実行中は開始しない(パネル競合防止)
    const errEl = document.getElementById('mysora-error');
    if (errEl) errEl.innerHTML = '';
    const checked = appState.mySoraSearches.filter(t => t.checked);
    if (!checked.length) return alert('一括計算するMy宙検索にチェックを入れてください');
    for (const t of checked) {
        const obs = appState.myObservations.find(o => o.id === t.obsId);
        const tgt = appState.myTargets.find(g => g.id === t.tgtId);
        if (!obs || !tgt) {
            if (errEl) errEl.innerHTML = `<span class="mypoint-error-text">宙検索ID:${t.id}の${!obs ? `観測点ID:${t.obsId}` : `目的点ID:${t.tgtId}`}がMyリストに存在しません。</span>`;
            return;
        }
    }
    if (fileMode && !confirm('チェックしたMy宙検索を一括で宙検索し、結果をCSVでFile取得しますか？')) return;
    _mySoraBatchRunning = true;
    _mySoraBatchCancel = false;
    const btn = document.getElementById(fileMode ? 'btn-mysora-file' : 'btn-mysora-batch');
    if (btn) btn.classList.add('active');
    const statusEl = document.getElementById('sorasearch-status');
    const setBusyBase = (t) => { if (statusEl) statusEl.innerHTML = `<span class="clock-anim">🕛</span> ${escapeHtml(t)}`; };
    try {
        openSoraSearchPanel();
        const titleEl = document.getElementById('sorasearch-title');
        if (titleEl) titleEl.textContent = '宙検索結果 (My宙検索一括)';
        document.getElementById('sorasearch-content').innerHTML = '';
        ssHideDetail();
        ssClearMapOverlay();
        _ssSelRow = null;
        const allRows = [];
        let errors = 0;
        for (let i = 0; i < checked.length; i++) {
            if (_mySoraBatchCancel) break;
            const t = checked[i];
            const obs = appState.myObservations.find(o => o.id === t.obsId);
            const tgt = appState.myTargets.find(g => g.id === t.tgtId);
            const label = t.name || `ID:${t.id}`;
            try {
                const snap = await ssRunSearchCore(mySoraEntryCond(t, obs, tgt), (msg) => setBusyBase(`(${i + 1}/${checked.length}) ${label}: ${msg}`));
                snap.rows.forEach(r => { r._entry = t; r._obs = obs; r._tgt = tgt; });
                allRows.push(...snap.rows);
            } catch (e) {
                console.error('My宙検索一括:', t.id, e);
                errors++;
            }
        }
        _ssLast = null;   // 一括の行はそれぞれ _snap を持つ(単発のスナップショットとは別管理)
        ssRenderResults({ rows: allRows, title: `宙検索結果 (My宙検索一括${_mySoraBatchCancel ? '・中断' : ''})`, batch: true });
        if (statusEl) statusEl.textContent = `(${allRows.length}件${errors ? `・${errors}件失敗` : ''})`;
        if (fileMode && allRows.length && !_mySoraBatchCancel) {
            setBusyBase(`CSV生成中… (${allRows.length}件)`);
            await mySoraDownloadCsv(allRows);
            if (statusEl) statusEl.textContent = `(${allRows.length}件・CSV出力完了)`;
        }
        if (!allRows.length && !_mySoraBatchCancel) alert('該当する時刻がありません(時間帯/検索期間/参照IDを見直してください)' + (errors ? `\n(${errors}件の検索でエラーが発生しました)` : ''));
    } finally {
        _mySoraBatchRunning = false;
        if (btn) btn.classList.remove('active');
    }
}

/** My宙検索一括のCSV(宙検索ID/名/メモ+観測点/目的点ID/名+結果18列+位置情報+評価尺度+検索条件) */
async function mySoraDownloadCsv(rows) {
    const header = ['宙検索ID', '宙検索名', '宙検索メモ', '観測点ID', '観測点名', '目的点ID', '目的点名'].concat(SS_LIST_HEADERS, [
        '観測点緯度', '観測点経度', '観測点標高', '観測点高', '目的点緯度', '目的点経度', '目的点標高', '目的点高',
        '狙い', '雲低重み', '雲中重み', '雲高重み', '月モード', '月重み', '雲海モード', '雲海重み', '光害重み', '透明度重み',
        '対象', '対象重み', '時間帯フィルタ', '検索期間(日)', '間隔(時間)', '扇角(°)', '範囲(km)', '標本格子数']);
    const esc = v => {
    ...
    for (let i = 0; i < rows.length; i++) {
        const r = rows[i], t = r._entry, obs = r._obs, tgt = r._tgt, snap = r._snap, p = snap.preset;
        const bandsStr = snap.bands ? Object.keys(SS_BAND_LABELS).filter(k => snap.bands[k]).map(k => SS_BAND_LABELS[k]).join('/') : '';
        const head7 = [t.id, t.name || '', t.memo || '', obs.id, obs.name || '', tgt.id, tgt.name || ''];
        const fixed = [
            obs.lat, obs.lng, obs.elev ?? 0, obs.height || 0, tgt.lat, tgt.lng, tgt.elev ?? 0, tgt.height || 0,
            p.label, p.wL, p.wM, p.wH,
            p.moon === 'avoid' ? '避ける' : '狙う', p.wMoon,
            p.unkai === 'avoid' ? '避ける' : '狙う', p.wUnkai, p.wLp, p.wTr,
            SS_OBJ_LABELS[p.obj] || p.obj, p.wObj, bandsStr,
            snap.days, snap.stepH, snap.fanDeg, snap.rangeKm, snap.points.length,
        ];
...
function parseMySoraCsvLine(cols, lineNum) {
    if (cols.length < 22) { alert(`${lineNum}行目: 列が不足しています(22列必要)`); return null; }
    const id = parseInt(toHalfWidth(cols[0].trim()));
    if (isNaN(id) || id < 1 || id > 1000) { alert(`${lineNum}行目: 宙検索IDが無効です(1〜1000)`); return null; }
    const name = cols[1].trim();
    const obsId = cols[2].trim() === '' ? null : parseInt(toHalfWidth(cols[2].trim()));
    const tgtId = cols[3].trim() === '' ? null : parseInt(toHalfWidth(cols[3].trim()));
    if (obsId !== null && (isNaN(obsId) || obsId < 1 || obsId > 1000)) { alert(`${lineNum}行目: 観測点IDが無効です(1〜1000)`); return null; }
    if (tgtId !== null && (isNaN(tgtId) || tgtId < 1 || tgtId > 1000)) { alert(`${lineNum}行目: 目的点IDが無効です(1〜1000)`); return null; }
    const labelToKey = Object.fromEntries(Object.entries(SS_PRESETS).map(([k, p]) => [p.label, k]));
    const presetLabel = cols[4].trim();
    let preset, weights = null;
    if (presetLabel === 'カスタム') {
        preset = 'custom';
        const wNum = (i, def) => { const v = parseFloat(toHalfWidth(cols[i].trim())); return isFinite(v) ? Math.max(0, Math.min(100, Math.round(v))) : def; };
        const objKey = Object.entries(SS_OBJ_LABELS).find(([, l]) => l === cols[14].trim());
        const bandLabels = cols[16].trim().split('/');
        const bands = {};
        Object.entries(SS_BAND_LABELS).forEach(([k, l]) => { bands[k] = bandLabels.includes(l); });
        if (!bands.night && !bands.twilight && !bands.ghbh && !bands.day) bands.night = true;
        weights = {
            wL: wNum(5, 100), wM: wNum(6, 85), wH: wNum(7, 40),
            moon: cols[8].trim() === '狙う' ? 'want' : 'avoid', wMoon: wNum(9, 80),
            unkai: cols[10].trim() === '狙う' ? 'want' : 'avoid', wUnkai: wNum(11, 0),
            wLp: wNum(12, 70), wTr: wNum(13, 40),
            obj: objKey ? objKey[0] : 'mw', wObj: wNum(15, 60), bands,
        };
    } else if (presetLabel in labelToKey) {
        preset = labelToKey[presetLabel];
    } else { alert(`${lineNum}行目: 狙い「${presetLabel}」が不明です(プリセット名かカスタム)`); return null; }
    const days = Math.max(1, Math.min(16, parseInt(toHalfWidth(cols[17].trim())) || 11));
    const interval = parseInt(toHalfWidth(cols[18].trim())) === 3 ? 3 : 1;
    const fan = Math.max(1, Math.min(180, parseFloat(toHalfWidth(cols[19].trim())) || 24));
    const rv = parseFloat(toHalfWidth(cols[20].trim()));
    const range = isFinite(rv) && rv > 0 ? Math.max(1, Math.min(300, rv)) : null;
    const memo = cols[21].trim();
    return { id, name, obsId, tgtId, preset, weights, days, interval, fan, range, checked: false, memo };
}
...
function setupMySoraControls() {
    const on = (id, fn) => { const el = document.getElementById(id); if (el) el.onclick = fn; };
    on('btn-mysora-toggle-all', () => {
        const anyUnchecked = appState.mySoraSearches.some(t => !t.checked);
        appState.mySoraSearches.forEach(t => { t.checked = anyUnchecked; });
        saveAppState();
        renderMySoraSearches();
    });
    on('btn-mysora-batch', () => runMySoraBatch(false));
    on('btn-mysora-file', () => runMySoraBatch(true));
    on('btn-mysora-get', getMySoraFromSoraSearch);
    on('btn-mysora-regall', registerAllMySora);
    on('btn-mysora-up', () => moveMySora(-1));
    on('btn-mysora-down', () => moveMySora(1));
    on('btn-mysora-addrow', addMySoraRow);
    on('btn-mysora-delrow', deleteMySoraRow);
    on('btn-mysora-csv-import', importMySoraCsv);
    on('btn-mysora-csv-append', appendMySoraCsv);
    on('btn-mysora-csv-export', exportMySoraCsv);
    renderMySoraSearches();
}
