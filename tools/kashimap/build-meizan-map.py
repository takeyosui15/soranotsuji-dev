...
OVERRIDES = {
    ...
    ('l100', '谷川岳'): ('293-3', '要確認',   '最高点オキノ耳1977m(Wikipedia表と同じ)。地理院は茂倉岳1978m・一ノ倉岳1974mも谷川岳の峰として持つ'),
    ('l100', '九重山'): ('926-1', '要確認',   '最高点=中岳1791m(Wikipedia表と同じ)。主峰は久住山(926-6・1786m)'),
    ('l100', '飯豊山'): ('228',   '要確認',   '地理院の飯豊山2105m(三角点)。Wikipedia表の2128mは連峰最高点の大日岳(229・新潟)'),
    ('l100', '那須岳'): ('248',   '要確認',   '連峰最高点=三本槍岳1917m(Wikipedia表と同じ)。主峰の茶臼岳は249(1915m)'),
    ('l100', '安達太良山'): ('211-1', '要確認', 'Wikipedia表の1709m=鉄山(211-1)。本峰(乳首)は211-2の1700m'),
    ('l100', '丹沢山'): ('361-1', '要確認',   '山塊の最高点=蛭ヶ岳1673m(Wikipedia表と同じ)。三角点の丹沢山は361-2(1567m)・塔ノ岳は361-3'),
    ...
    ('l300', '吾妻山'): (None, '地理院に無し', '広島県庄原市の吾妻山1238m。Wikipediaの座標を仮置き(近くの比婆山＜烏帽子山＞802-2とは別の山)'),
    ('l300', '二岐山'): (None, '地理院に無し', '福島県。Wikipediaの座標を仮置き'),
    ('l300', '諏訪山'): (None, '地理院に無し', '群馬県。Wikipediaの座標を仮置き'),
    ('l300', '藤原岳'): (None, '地理院に無し', '三重県/滋賀県。Wikipediaの座標を仮置き'),
}
